...
local function drop(tab,t,flag,vals,val,cb) local d; pcall(function() d=tab:Dropdown({Title=L(t),Desc="",Flag=flag,Values=vals,Value=val,Callback=cb}) end); if d then function d:SetValues(v) self.Values=v; if self.Refresh then self:Refresh(v) end end end; return d endlocal function col(tab,t,flag,def,cb)pcall(function()tab:Colorpicker({Title=L(t),Desc="",Flag=flag,Default=def,Transparency=0,Locked=false,Callback=cb})end)end
...
section(Tabs.OP,"Utility");local OPPlayerDrop=drop(Tabs.OP,"SelectPlayer","OPSelectedPlayer",playerNames(),"None",function(v)SelectedPlayer=v end);btn(Tabs.OP,"RefreshPlayers",function() local novaLista=playerNames(); if OPPlayerDrop then pcall(function() OPPlayerDrop:SetValues(novaLista) end) end; notify(L("PlayerListUpdated"),"refresh-cw",2) end);btn(Tabs.OP,"TeleportPlayer",tpToPlayer);btn(Tabs.OP,"TeleportBack",teleportBack);btn(Tabs.OP,"BringPlayer",bringPlayer);btn(Tabs.OP,"SpectatePlayer",spectatePlayer);btn(Tabs.OP,"Unspectate",unspectate)
...
section(Tabs.Troll,"Movement");local TrollPlayerDrop = drop(Tabs.Troll,"SelectPlayer","TrollSelectedPlayer",playerNames(),"None",function(v)SelectedPlayer=v end);btn(Tabs.Troll,"RefreshPlayers",function() local novaLista = playerNames(); if TrollPlayerDrop and TrollPlayerDrop.Dropdown then pcall(function() TrollPlayerDrop.Dropdown:SetValues(novaLista) end) end; notify(L("PlayerListUpdated"),"refresh-cw",2) end);tog(Tabs.Troll,"Orbit","TrollOrbit",false,function(v)TrollC.Orbit=v end);drop(Tabs.Troll,"OrbitMode","TrollOrbitMode",{"Circle","UpDown","LeftRight","Diagonal"},"Circle",function(v)TrollC.OrbitMode=v end);slid(Tabs.Troll,"OrbitRadius","TrollOrbitRadius",2,35,TrollC.OrbitRadius,1,function(v)TrollC.OrbitRadius=v end);slid(Tabs.Troll,"OrbitSpeed","TrollOrbitSpeed",1,40,TrollC.OrbitSpeed,1,function(v)TrollC.OrbitSpeed=v end);slid(Tabs.Troll,"OrbitHeight","TrollOrbitHeight",0,25,TrollC.OrbitHeight,1,function(v)TrollC.OrbitHeight=v end);tog(Tabs.Troll,"SpinBot","TrollSpinBot",false,function(v)TrollC.Spin=v end);slid(Tabs.Troll,"SpinSpeed","TrollSpinSpeed",1,150,TrollC.SpinSpeed,1,function(v)TrollC.SpinSpeed=v end);tog(Tabs.Troll,"HeadSit","TrollHeadSit",false,function(v)TrollC.HeadSit=v;if not v then stopHeadSit()end end)
