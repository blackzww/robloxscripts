...
HomeTitle="Mirrors Hub",HomeDesc="Universal script hub.\nVersion: 1.3\nCreated by blackzw.mp3",BetaTitle="Beta Notice",BetaDesc="This hub is still in beta and may contain bugs. Report issues on Discord.",DiscordTitle="Community",DiscordDesc="Found bugs or have suggestions? Send them on Discord.",StatusTitle="Status",StatusDesc="Build: 1.3 Beta.",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersão: 1.3\nCriado por blackzw.mp3",BetaTitle="Aviso Beta",BetaDesc="Este hub ainda está em beta e pode conter bugs. Avise no Discord.",DiscordTitle="Comunidade",DiscordDesc="Achou bugs ou tem sugestões? Manda no Discord.",StatusTitle="Status",StatusDesc="Build: 1.3 Beta.",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersión: 1.3\nCreado por blackzw.mp3",BetaTitle="Aviso Beta",BetaDesc="Este hub todavía está en beta y puede tener bugs. Avísalo en Discord.",DiscordTitle="Comunidad",DiscordDesc="¿Bugs o sugerencias? Mándalos en Discord.",StatusTitle="Estado",StatusDesc="Build: 1.3 Beta.",
...
local Tabs={Scripts=Window:Tab({Title=L("Scripts"),Icon="scroll-text"}),Aimbot=Window:Tab({Title=L("Aimbot"),Icon="crosshair"}),ESP=Window:Tab({Title=L("ESP"),Icon="eye"}),Misc=Window:Tab({Title=L("Misc"),Icon="circle-ellipsis"}),OP=Window:Tab({Title=L("OP"),Icon="flame",Locked=false}),Troll=Window:Tab({Title=L("Troll"),Icon="laugh"}),Config=Window:Tab({Title=L("Config"),Icon="cog"}),Status=Window:Tab({Title=L("Status"),Icon="activity"}),Info=Window:Tab({Title=L("Info"),Icon="info"}),IA=Window:Tab({Title="Assistente IA",Icon="bot"})}
...
-- ==== Mirrors Hub AI ====
local MIRRORS_API_URL = "https://mirrors-hub-ai.vercel.app/api/ask"
local MIRRORS_API_KEY = "mh_9f8a7s6d5f4g3h2j1k"
local ultimoPedidoIA = 0
local INTERVALO_MINIMO_IA = 3
local function PerguntarIA(pergunta, callback)
	local agora = tick()
	if agora - ultimoPedidoIA < INTERVALO_MINIMO_IA then
		callback(false, "Aguarde um instante antes de perguntar novamente.")
		return
	end
	ultimoPedidoIA = agora

	local req = nil
	pcall(function() req = request or http_request or (syn and syn.request) end)
	if type(req) ~= "function" then
		callback(false, "Seu executor não suporta requisições HTTP.")
		return
	end

	local corpo = Http:JSONEncode({
		question = pergunta,
		userId = tostring(LP.UserId)
	})
	...
	if not sucesso then
		callback(false, "Erro de conexão com o servidor de IA.")
		return
	end

	local status = resultado.StatusCode or resultado.Status
	local ok, dados = pcall(function() return Http:JSONDecode(resultado.Body) end)
	if not ok then
		callback(false, "Resposta inválida do servidor.")
		return
	end

	if status == 200 and dados.resposta then
		callback(true, dados.resposta)
	else
		callback(false, dados.erro or "Não foi possível obter uma resposta agora.")
	end
end
...
section(Tabs.IA,"Assistente")
local RespostaIA = nil
pcall(function()
	RespostaIA = Tabs.IA:Paragraph({Title="Resposta",Desc="Digite uma pergunta abaixo e aperte Enter.",Color="Blue",Locked=false})
end)
input(Tabs.IA,"Pergunte algo sobre o Mirrors Hub","Ex: Como funciona o Aimbot?",function(pergunta)
	if not pergunta or tostring(pergunta):gsub("%s","")=="" then return end
	pcall(function() if RespostaIA then RespostaIA:SetDesc("Pensando...") end end)
	PerguntarIA(tostring(pergunta), function(sucesso, resposta)
		pcall(function()
			if RespostaIA then
				if sucesso then RespostaIA:SetDesc(resposta)
				else RespostaIA:SetDesc("Erro: "..resposta) end
			end
		end)
	end)
end)
