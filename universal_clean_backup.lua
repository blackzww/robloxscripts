...
section(Tabs.OP,"Utility");local OPPlayerDrop = drop(Tabs.OP,"SelectPlayer","OPSelectedPlayer",playerNames(),"None",function(v)SelectedPlayer=v end);btn(Tabs.OP,"RefreshPlayers",function() local novaLista = playerNames(); if OPPlayerDrop then pcall(function() OPPlayerDrop:SetOptions(novaLista) end); pcall(function() OPPlayerDrop:Refresh(novaLista, true) end); pcall(function() if OPPlayerDrop.Dropdown then OPPlayerDrop.Dropdown:SetOptions(novaLista) elseif OPPlayerDrop.Element then OPPlayerDrop.Element:SetOptions(novaLista) end end) end; notify(L("PlayerListUpdated"),"refresh-cw",2) end);btn(Tabs.OP,"TeleportPlayer",tpToPlayer);btn(Tabs.OP,"TeleportBack",teleportBack);btn(Tabs.OP,"BringPlayer",bringPlayer);btn(Tabs.OP,"SpectatePlayer",spectatePlayer);btn(Tabs.OP,"Unspectate",unspectate)
