...
section(Tabs.OP,"Utility");local OPPlayerDrop = drop(Tabs.OP,"SelectPlayer","OPSelectedPlayer",playerNames(),"None",function(v)SelectedPlayer=v end);btn(Tabs.OP,"RefreshPlayers",function() local novaLista = playerNames(); if OPPlayerDrop then if OPPlayerDrop.SetOptions then OPPlayerDrop:SetOptions(novaLista) elseif OPPlayerDrop.Refresh then OPPlayerDrop:Refresh(novaLista, true) end end; notify(L("PlayerListUpdated"),"refresh-cw",2) end);btn(Tabs.OP,"TeleportPlayer",tpToPlayer);btn(Tabs.OP,"TeleportBack",teleportBack);btn(Tabs.OP,"BringPlayer",bringPlayer);btn(Tabs.OP,"SpectatePlayer",spectatePlayer);btn(Tabs.OP,"Unspectate",unspectate)
