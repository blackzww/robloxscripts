...
local function drop(tab,t,flag,vals,val,cb) local d; pcall(function() d = tab:Dropdown({Title=L(t),Desc="",Flag=flag,Values=vals,Value=val,Callback=cb}) end) if d and d.Dropdown then return d.Dropdown else return d end end
...
section(Tabs.OP,"Utility");local OPPlayerDrop = drop(Tabs.OP,"SelectPlayer","OPSelectedPlayer",playerNames(),"None",function(v)SelectedPlayer=v end);btn(Tabs.OP,"RefreshPlayers",function() local novaLista = playerNames(); if OPPlayerDrop and OPPlayerDrop.SetValues then pcall(function() OPPlayerDrop:SetValues(novaLista) end) end; notify(L("PlayerListUpdated"),"refresh-cw",2) end);btn(Tabs.OP,"TeleportPlayer",tpToPlayer);btn(Tabs.OP,"TeleportBack",teleportBack);btn(Tabs.OP,"BringPlayer",bringPlayer);btn(Tabs.OP,"SpectatePlayer",spectatePlayer);btn(Tabs.OP,"Unspectate",unspectate)
