...
do
    ...
    if Env.MirrorsMM2V51Runtime and Env.MirrorsMM2V51Runtime.Cleanup then
        pcall(Env.MirrorsMM2V51Runtime.Cleanup)
    end
    ...
    Env.MirrorsMM2V51Runtime = Runtime
...
["Your executor does not support file configs."] = "Tu executor no soporta configuraciones en archivo.",
    ...
    local function Translate(Text, Language)
        ...
        if Language == "Português" then
            local Extra = {
                ...
                ["Mirrors Purple V5.1 loaded."] = "Mirrors Purple V5.1 carregado."
            }
        ...
        elseif Language == "Español" then
            local Extra = {
                ...
                ["Visual reapplied."] = "Visual reaplicado.",
                ["Mirrors Purple V5.1 loaded."] = "Mirrors Purple V5.1 cargado."
            }
            return Extra[Text] or Text
        end
    ...
    local function ButtonVisual(Title)
        ...
        if Lower:find("delete", 1, true) or Lower:find("kill", 1, true) then
            return C.Red, "solar:cursor-square-bold"
        elseif Lower:find("server", 1, true) or Lower:find("rejoin", 1, true) or Lower:find("teleport", 1, true) or Lower:find("tp ", 1, true) then
            return C.Cyan, "solar:square-transfer-horizontal-bold"
        elseif Lower:find("save", 1, true) or Lower:find("export", 1, true) then
            return C.Purple, "solar:file-text-bold"
        elseif Lower:find("load", 1, true) or Lower:find("import", 1, true) then
            return C.Blue, "solar:folder-with-files-bold"
        elseif Lower:find("refresh", 1, true) or Lower:find("stop", 1, true) then
            return C.Orange, "solar:cursor-square-bold"
        elseif Lower:find("fling", 1, true) then
            return C.PurpleVivid, "solar:square-transfer-horizontal-bold"
        end
        return C.Purple, "solar:cursor-square-bold"
    end
            ...
            if typeof(Original) == "function" then
                Tab[TypeName] = function(Self, Options)
                    ...
                    if TypeName == "Button" then
                        local Color, Icon = ButtonVisual(RawTitle)
                        New.Color = New.Color or Color
                        New.Icon = Icon
                        New.IconAlign = "Left"
                        New.Justify = New.Justify or "Center"
                    end
                    ...
                    StyleToken += 1
                    local Token = StyleToken
                    task.delay(0.35, function()
                        if Runtime.Alive and Token == StyleToken and Mirrors.ApplyDetailedStyle then
    ...
    local InitialTheme = ThemeKeys[State.Theme] or "Mirrors Purple"
    pcall(function()
    ...
    local Tabs = {
        Info = CreateTab(General, "Info", "Overview", "solar:info-square-bold"),
        Main = CreateTab(General, "Main", "Gameplay", "solar:home-2-bold"),
        ESP = CreateTab(General, "ESP", "Visuals", "solar:cursor-square-bold"),
        Murder = CreateTab(RolesSection, "Murder", "Role tools", "solar:cursor-square-bold"),
        Sheriff = CreateTab(RolesSection, "Sheriff", "Defense", "solar:check-square-bold"),
        Innocent = CreateTab(RolesSection, "Innocent", "Survival", "solar:info-square-bold"),
        Shortcuts = CreateTab(System, "Shortcuts", "Quick actions", "solar:cursor-square-bold"),
        Misc = CreateTab(System, "Misc", "Utilities", "solar:file-text-bold"),
        Config = CreateTab(System, "Config", "Settings", "solar:folder-with-files-bold")
    }
    ...
    local function ApplyWindowVisual()
        ...
        CancelVisualTweens()
        for _, Name in ipairs({ "MirrorsV51Background", "MirrorsV51Tint", "MirrorsV51Shade", "MirrorsV51Border", "MirrorsV51Glow", "MirrorsV51Accent" }) do
        ...
        AddWhiteGlow(Accent, "MirrorsV51AccentGlow", 6, 0.58)
    ...
    local function StyleOpenButton()
        local Button = FindButton(Translate("Open Mirrors Hub - MM2")) or FindButton("Open Mirrors Hub - MM2")
        if not Button then
            return
        end

        if not Button:FindFirstChild("MirrorsV51Scale") then
            local Scale = Instance.new("UIScale")
            Scale.Name = "MirrorsV51Scale"
            Scale.Scale = 1
            Scale.Parent = Button

            local Glow = AddWhiteGlow(Button, "MirrorsV51OpenGlow", 6, 0.52)

            local Dot = Instance.new("Frame")
            Dot.Name = "MirrorsV51Dot"
            ...
            AddWhiteGlow(Dot, "MirrorsV51DotGlow", 6, 0.48)
    ...
    local function ClearSelectedGlow()
        for _, Button in pairs(TabButtons) do
            RemoveNamed(Button, "MirrorsV51SelectedEdge")
            RemoveNamed(Button, "MirrorsV51SelectedGlow")
        end
    end
    ...
    local function ApplySelectedGlow(Key, Button)
        ...
        local Edge = AddWhiteGlow(Button, "MirrorsV51SelectedEdge", 1.5, 0.12)
        local Glow = AddWhiteGlow(Button, "MirrorsV51SelectedGlow", 5, 0.58)
    ...
    local function StyleTab(Key)
        ...
        if IconContainer then
            ...
            IconContainer.BackgroundTransparency = 0.62
            local Corner = IconContainer:FindFirstChildOfClass("UICorner")
            if Corner then
                Corner.CornerRadius = UDim.new(0, 9)
            end
            local Edge = AddWhiteGlow(IconContainer, "MirrorsV51IconEdge", 1.2, 0.14)
            if Edge then Edge.Color = Color end
            AddWhiteGlow(IconContainer, "MirrorsV51IconGlow", 5, 0.62)
            for _, Child in ipairs(IconContainer:GetDescendants()) do
                if Child:IsA("ImageLabel") or Child:IsA("ImageButton") then
                    Child.ImageColor3 = C.White
                end
            end
        end

        if not Button:FindFirstChild("MirrorsV51TabBound") then
            local Marker = Instance.new("BoolValue")
            Marker.Name = "MirrorsV51TabBound"
    ...
    local function FindSmallestFrameForText(TextObject)
        local Parent = TextObject and TextObject.Parent
        local Best
        local BestArea = math.huge
        for _ = 1, 7 do
            if not Parent then
            ...
            if Parent:IsA("Frame") then
                local Size = Parent.AbsoluteSize
                if Size.X >= 220 and Size.Y >= 28 and Size.Y <= 240 then
                    local Area = Size.X * Size.Y
                    if Area < BestArea then
                        Best = Parent
                        BestArea = Area
    ...
    local function StyleRegisteredElements()
        for _, Entry in ipairs(Registry) do
            if Entry.Type ~= "Tab" and typeof(Entry.Title) == "string" then
                local TextObject = FindText(Translate(Entry.Title)) or FindText(Entry.Title)
                local Frame = FindSmallestFrameForText(TextObject)
                if Frame and not Frame:FindFirstChild("MirrorsV51ElementGlow") then
                    local Glow = AddWhiteGlow(Frame, "MirrorsV51ElementGlow", 2.5, 0.88)
                    local Bound = Instance.new("BoolValue")
                    Bound.Name = "MirrorsV51HoverBound"
                    Bound.Parent = Frame
                    TrackConnection(Frame.MouseEnter:Connect(function()
                        if Glow and Glow.Parent then
                            TweenService:Create(Glow, TweenInfo.new(0.16), { Transparency = 0.64 }):Play()
                        end
                    end))
                    TrackConnection(Frame.MouseLeave:Connect(function()
                        if Glow and Glow.Parent then
                            TweenService:Create(Glow, TweenInfo.new(0.20), { Transparency = 0.88 }):Play()
                        end
                    end))
                end

                if Entry.Type == "Button" then
                    local Button = FindButton(Translate(Entry.Title)) or FindButton(Entry.Title)
                    if Button and not Button:FindFirstChild("MirrorsV51ButtonScale") then
                        local Scale = Instance.new("UIScale")
                        Scale.Name = "MirrorsV51ButtonScale"
                        Scale.Scale = 1
                        Scale.Parent = Button
                        local Glow = AddWhiteGlow(Button, "MirrorsV51ButtonGlow", 4.5, 0.70)
                        TrackConnection(Button.Activated:Connect(function()
                            TweenService:Create(Scale, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1.055 }):Play()
                            if Glow then
                                TweenService:Create(Glow, TweenInfo.new(0.10), { Transparency = 0.25 }):Play()
                            end
                            task.delay(0.14, function()
                                if Scale.Parent then
                                    TweenService:Create(Scale, TweenInfo.new(0.20, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
                                    if Glow and Glow.Parent then
                                        TweenService:Create(Glow, TweenInfo.new(0.28), { Transparency = 0.70 }):Play()
                                    end
                                end
                            end)
                        end))
    ...
    function Mirrors.RefreshLanguage()
        for _, Entry in ipairs(Registry) do
            ...
            if Element then
                ...
                if typeof(Entry.Desc) == "string" and Element.SetDesc then
                    pcall(function()
    ...
    Runtime.Cleanup = function()
        ...
        if Env.MirrorsMM2V51Runtime == Runtime then
            Env.MirrorsMM2V51Runtime = nil
        end
    end
    ...
    task.delay(1, function()
        if Runtime.Alive then
            State.Ready = true
            Mirrors.ApplyDetailedStyle()
            Notify("Mirrors Hub - MM2", "Mirrors Purple V5.1 loaded.", "solar:check-square-bold", 3)
        end
    end)
end
...
local function FindRolePlayer(PrimaryRole, SecondaryRole)
    UpdateRoles()
    ...
    for _, Player in ipairs(Players:GetPlayers()) do
        if Player ~= LP then
            ...
            if Role == PrimaryRole
            or (SecondaryRole and Role == SecondaryRole) then
...
local function TeleportToRole(PrimaryRole, SecondaryRole)
    local Target = FindRolePlayer(PrimaryRole, SecondaryRole)
    local Root = GetRoot()
    ...
    if not Root or not TargetRoot then
        Mirrors.Notify("No Target", "No valid role target was found.", "solar:info-square-bold", 2)
        return
    end
...
local function FlingRole(PrimaryRole, SecondaryRole)
    local Target = FindRolePlayer(PrimaryRole, SecondaryRole)
    ...
    FollowAndFlingPlayer(Target)
end

MurderTab:Paragraph({
    Title = "Role Tools",
    Desc = "Visual and movement utilities for the current round."
})

MurderTab:Toggle({
    Title = "Sheriff ESP",
    Desc = "Control Sheriff and Hero visibility in the main ESP system.",
    Value = true,
    Callback = function(Value)
...
MurderTab:Button({
    ...
    Callback = function()
        FlingRole("Sheriff", "Hero")
    end
})

SheriffTab:Paragraph({
    Title = "Role Tools",
    Desc = "Visual and movement utilities for the current round."
})
...
SheriffTab:Toggle({
    Title = "Murderer ESP",
    Desc = "Control Murderer visibility in the main ESP system.",
    Value = true,
    Callback = function(Value)
        ESPSettings.Murderer = Value
        RefreshESP()
    end
})
...
ShortcutsTab:Paragraph({
    Title = "Quick Actions",
    Desc = "Fast movement shortcuts for the current round."
})
...
ConfigTab:Paragraph({
    ...
    Desc = "Language, theme and notification preferences."
})
