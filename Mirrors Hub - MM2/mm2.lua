...
--==================================================
-- MIRRORS HUB KEY SYSTEM
--==================================================
if not (function()
    local Env = getgenv and getgenv() or _G

    -- A re-execution must never leave an already-open hub running
    -- while a new key validation is pending.
    local PreviousHub = Env.MirrorsMM2Runtime
    if PreviousHub and PreviousHub.Cleanup then
        pcall(PreviousHub.Cleanup)
    end

    local PreviousKeyRuntime = Env.MirrorsMM2KeyRuntime
    if PreviousKeyRuntime and PreviousKeyRuntime.Cleanup then
        pcall(PreviousKeyRuntime.Cleanup)
    end
    ...
    Env.MirrorsMM2KeyRuntime = KeyRuntime

    local API = "https://mirrorshub-key.vercel.app/api/key/validate"
    local GET_KEY_URL = "https://mirrorshub-key.vercel.app"
    local KEY_FILE = "WindUI/MirrorsHub/MM2/key.json"
    ...
    local KeyMessages = {
        EMPTY_KEY = "Enter your Mirrors Hub key.",
        HTTP_UNSUPPORTED = "This executor does not support HTTP requests.",
        CONNECTION_ERROR = "Unable to connect to the Mirrors Hub server.",
        ...
        INVALID_ROBLOX_USER = "Invalid Roblox user.",
        ROBLOX_USER_NOT_FOUND = "Roblox account could not be verified.",
        ROBLOX_USER_MISMATCH = "This key is linked to another Roblox account.",
        ...
        INVALID_REQUEST = "Invalid validation request.",
        ACCESS_DENIED = "Access denied.",
        USER_BANNED = "Access denied.",
        VALIDATION_ERROR = "Unable to validate key. Please try again."
    }

    local function CleanupKeyRuntime()
        if not KeyRuntime.Alive then
            return
        end

        KeyRuntime.Alive = false

        if KeyRuntime.Window then
            local Window = KeyRuntime.Window
            KeyRuntime.Window = nil
            pcall(function()
                Window:Destroy()
            end)
        end

        if Env.MirrorsMM2KeyRuntime == KeyRuntime then
            Env.MirrorsMM2KeyRuntime = nil
        end
    end

    KeyRuntime.Cleanup = CleanupKeyRuntime
    ...
    local function ResolveRequest()
        ...
        if type(Fluxus) == "table" and typeof(Fluxus.request) == "function" then
    ...
    local function GetExecutor()
        local Identify = rawget(Env, "identifyexecutor")
        if typeof(Identify) == "function" then
        ...
        local GetName = rawget(Env, "getexecutorname")
        if typeof(GetName) == "function" then
    ...
    local function MaskHWID(Value)
        ...
        local Stars = math.clamp(Length - 8, 6, 18)
        return Value:sub(1, 4) .. string.rep("*", Stars) .. Value:sub(-4)
    end
    ...
    local function FormatProvider(Provider)
        ...
        return Names[tostring(Provider or ""):upper()]
            or tostring(Provider or "Unknown")
    end
    ...
    local function RemainingTime(Iso)
        ...
        local Success, Expires = pcall(DateTime.fromIsoDate, tostring(Iso))
        if not Success or not Expires then
            return "Unknown"
        end

        local Seconds = math.floor(
            (Expires.UnixTimestampMillis - DateTime.now().UnixTimestampMillis) / 1000
        )
    ...
    local function EnsureKeyFolder()
        ...
        for _, Folder in ipairs({
            "WindUI",
            "WindUI/MirrorsHub",
            "WindUI/MirrorsHub/MM2"
        }) do
            if not isfolder(Folder) then
                local Success = pcall(makefolder, Folder)
                if not Success and not isfolder(Folder) then
                    return false
                end
            end
        end
    ...
    local function LoadSavedKey()
        ...
        local Success, Raw = pcall(readfile, KEY_FILE)
        if not Success or type(Raw) ~= "string" or Raw == "" then
            return nil
        end

        local DecodedOK, Data = pcall(HttpService.JSONDecode, HttpService, Raw)
        if DecodedOK and type(Data) == "table" and type(Data.key) == "string" then
            return Data.key:match("^%s*(.-)%s*$")
        end

        -- Compatibility with a possible older plain-text key file.
    ...
    local function ClearSavedKey()
        if typeof(isfile) ~= "function" or not isfile(KEY_FILE) then
            return
        end
    ...
    local function Result(Code, Data)
        Code = tostring(Code or "VALIDATION_ERROR")
        return false, {
            code = Code,
            message = KeyMessages[Code] or Code,
            data = Data
        }
    end
    ...
    local function ValidateKey(Key)
        ...
        if Key == "" then
            return Result("EMPTY_KEY")
        end

        if not HWID then
            return Result("INVALID_HWID")
        end

        if not Request then
            return Result("HTTP_UNSUPPORTED")
        end
        ...
        if Data.valid == true then
            ...
            KeyInfo.Code = tostring(Data.code or "KEY_VALID")
            KeyInfo.Provider = FormatProvider(Data.provider)
        ...
        KeyInfo.Status = Code
        KeyInfo.Code = Code
        KeyInfo.Provider = "Unknown"
        KeyInfo.ExpiresAt = nil
    ...
    -- Saved keys are always checked again by the server. Nothing is trusted
    -- just because it exists on disk.
    if SavedKey and SavedKey ~= "" then
        KeyInfo.Status = "Checking saved key..."

        local Valid, Validation = ValidateKey(SavedKey)
        if Valid then
            KeyRuntime.Granted = true
            CleanupKeyRuntime()
            return true
        end

        local Code = Validation and Validation.code or "VALIDATION_ERROR"
        if Code == "INVALID_KEY"
        or Code == "KEY_EXPIRED"
        or Code == "HWID_MISMATCH"
        or Code == "ROBLOX_USER_MISMATCH" then
            ClearSavedKey()
            SavedKey = nil
        end
    end

    local WindUIOK, WindUI = pcall(function()
        return loadstring(game:HttpGet(
            "https://github.com/Footagesus/WindUI/releases/download/1.6.66/main.lua"
        ))()
    end)

    if not WindUIOK or not WindUI then
        warn("[MIRRORS] Unable to load the Key System interface.")
        CleanupKeyRuntime()
        return false
    end
    ...
    local KeyWindow = WindUI:CreateWindow({
        Title = "Mirrors Hub - Key System",
        Icon = "key",
        Author = "by blackzw.mp3",
        Folder = "MirrorsHub/KeySystem",
    ...
    KeyRuntime.Window = KeyWindow

    pcall(function()
        KeyWindow:EditOpenButton({
            Title = "Open Mirrors Hub - Key System",
            Icon = "key",
            CornerRadius = UDim.new(0, 16),
            StrokeThickness = 1.5,
            Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromHex("#F5E7FF")),
                ColorSequenceKeypoint.new(0.45, Color3.fromHex("#B93EFF")),
    ...
    pcall(function()
        KeyWindow:Tag({
            Title = "v1.6.1",
            Icon = "key",
            Color = Color3.fromHex("#AD35FF"),
            Border = true
        })
    end)

    local AccessSection = KeyWindow:Section({
        Title = "ACCESS",
        Opened = true
    })

    local InfoTab = AccessSection:Tab({
        Title = "Info",
        Desc = "Access information",
        Icon = "info",
        IconColor = Color3.fromHex("#C463FF"),
        IconShape = "Square",
        Border = true
    })

    local KeyTab = AccessSection:Tab({
        Title = "Key",
        Desc = "Validate access",
        Icon = "key",
        IconColor = Color3.fromHex("#43E39B"),
        IconShape = "Square",
        Border = true
    })
    ...
    local function UpdateInfoParagraph()
        if not InfoParagraph then
            return
        end

        local Description = table.concat({
            "Status: " .. tostring(KeyInfo.Status or "NO KEY"),
            "Provider: " .. tostring(KeyInfo.Provider or "Unknown"),
            "Remaining: " .. RemainingTime(KeyInfo.ExpiresAt),
            "Username: " .. tostring(LP.Name),
            "Display Name: " .. tostring(LP.DisplayName),
        ...
        pcall(function()
            InfoParagraph:SetDesc(Description)
        end)
    end
    ...
    KeyTab:Paragraph({
        Title = "Mirrors Hub Access",
        Desc = "Paste your key below. Saved keys are validated again every time the hub starts."
    })

    local CurrentKey = SavedKey or ""
    local Validating = false

    local KeyInput = KeyTab:Input({
        Title = "Access Key",
        Desc = "Your Mirrors Hub key",
        Value = CurrentKey,
        Placeholder = "MIRRORS-...",
        Callback = function(Value)
            CurrentKey = tostring(Value or ""):match("^%s*(.-)%s*$")
        end
    })
    ...
    local function FinishWithAccess(Key)
        SaveKey(Key)
        KeyRuntime.Granted = true
        Notify(
            "Access Granted",
    ...
    local function SubmitKey()
        if Validating or not KeyRuntime.Alive then
            return
        end

        CurrentKey = tostring(CurrentKey or ""):match("^%s*(.-)%s*$")
        ...
        local Valid, Validation = ValidateKey(CurrentKey)
        Validating = false
        UpdateInfoParagraph()

        if Valid then
            FinishWithAccess(CurrentKey)
            return
        end

        local Message = Validation and Validation.message or "Unable to validate key."
        Notify("Access Denied", Message, "x", 4)
    end

    KeyTab:Button({
        Title = "Validate Key",
        Icon = "check",
        Color = Color3.fromHex("#43E39B"),
        Justify = "Center",
        Callback = SubmitKey
    })
    ...
    KeyTab:Button({
        ...
        Callback = function()
            ...
            if typeof(Clipboard) == "function" then
                Copied = pcall(Clipboard, GET_KEY_URL)
            end

            if Copied then
                Notify("Get Key", "Link copied to clipboard.", "copy", 3)
            else
                Notify("Get Key", GET_KEY_URL, "external-link", 5)
            end
        end
    })

    if KeyWindow.OnDestroy then
        KeyWindow:OnDestroy(function()
            if KeyRuntime.Alive and not KeyRuntime.Granted then
                KeyRuntime.Alive = false
                KeyRuntime.Window = nil
                if Env.MirrorsMM2KeyRuntime == KeyRuntime then
                    Env.MirrorsMM2KeyRuntime = nil
    ...
    -- Keep the loader paused here. The MM2 hub is created only after
    -- a successful server-side validation.
    while KeyRuntime.Alive and not KeyRuntime.Granted do
        task.wait(0.1)
    end

    local Granted = KeyRuntime.Granted == true
    CleanupKeyRuntime()
    return Granted
end)() then
    return
...
if not Remotes then
    error("[MIRRORS] MM2 remotes were not found. Make sure you are inside Murder Mystery 2.")
end

local Gameplay = Remotes:WaitForChild("Gameplay", 10)
local Extras = Remotes:WaitForChild("Extras", 10)
if not Gameplay or not Extras then
    error("[MIRRORS] MM2 remote folders are unavailable in this server.")
end

local GetCurrentPlayerData = Gameplay:WaitForChild("GetCurrentPlayerData", 10)
local GetChance = Extras:WaitForChild("GetChance", 10)
if not GetCurrentPlayerData or not GetChance then
    error("[MIRRORS] Required MM2 remotes are unavailable.")
end
...
do
    ...
    ThemeKeys["Monokai Pro"] = "MonokaiPro"
    ThemeKeys["Cotton Candy"] = "CottonCandy"
    ...
    local WindUIOK, WindUI = pcall(function()
        ...
        if not Loader then
            error("WindUI compile error: " .. tostring(CompileError))
        end
    ...
    if not WindUIOK or not WindUI then
        error("[MIRRORS] Unable to load WindUI: " .. tostring(WindUI))
    end
...
["Settings File"] = "Arquivo settings.json",
["Reload settings.json"] = "Recarregar settings.json",
["Save Current settings.json"] = "Salvar settings.json Atual",
["Reset settings.json"] = "Redefinir settings.json",
["Copy Settings Path"] = "Copiar Caminho das Configurações",
["Path copied to clipboard."] = "Caminho copiado para a área de transferência.",
["Current settings saved."] = "Configurações atuais salvas.",
["Default settings file restored."] = "Arquivo settings.json padrão restaurado.",
["Settings applied successfully."] = "Configurações aplicadas com sucesso.",
["Server & round"] = "Servidor e rodada",
["Automation & players"] = "Automação e jogadores",
["Role visuals"] = "Visuais de funções",
["Role actions"] = "Ações da função",
["Quick panel"] = "Painel rápido",
["Server & player"] = "Servidor e jogador",
["Hub & files"] = "Hub e arquivos",
...
["Settings File"] = "Archivo settings.json",
["Reload settings.json"] = "Recargar settings.json",
["Save Current settings.json"] = "Guardar settings.json Actual",
["Reset settings.json"] = "Restablecer settings.json",
["Copy Settings Path"] = "Copiar Ruta de Configuración",
["Path copied to clipboard."] = "Ruta copiada al portapapeles.",
["Current settings saved."] = "Configuración actual guardada.",
["Default settings file restored."] = "Archivo settings.json predeterminado restaurado.",
["Settings applied successfully."] = "Configuración aplicada correctamente.",
["Server & round"] = "Servidor y ronda",
["Automation & players"] = "Automatización y jugadores",
["Role visuals"] = "Visuales de roles",
["Role actions"] = "Acciones del rol",
["Quick panel"] = "Panel rápido",
["Server & player"] = "Servidor y jugador",
["Hub & files"] = "Hub y archivos",
...
["Your executor does not support file configs."] = "Tu executor no soporta configuraciones en archivo.",
    ...
    local function ButtonVisual(Title)
        ...
        if Lower:find("delete", 1, true) then return C.Red end
        if Lower:find("kill", 1, true) then return C.Red end
        if Lower:find("stop", 1, true) or Lower:find("cancel", 1, true) then return Color3.fromHex("#FF6B8A") end
        ...
        if Lower:find("rejoin", 1, true) or Lower:find("server hop", 1, true) or Lower:find("small server", 1, true) then return C.Blue end
        if Lower:find("validate", 1, true) or Lower:find("save", 1, true) then return C.Green end
        if Lower:find("load", 1, true) then return Color3.fromHex("#6E7CFF") end
        if Lower:find("import", 1, true) then return Color3.fromHex("#7A6CFF") end
        if Lower:find("export", 1, true) or Lower:find("copy", 1, true) then return C.Lavender end
        if Lower:find("reset", 1, true) then return C.Yellow end
        if Lower:find("refresh", 1, true) or Lower:find("reapply", 1, true) then return Color3.fromHex("#63C7FF") end
        if Lower:find("center", 1, true) then return C.PurpleLight end
        if Lower:find("get key", 1, true) or Lower:find("open", 1, true) then return Color3.fromHex("#B95CFF") end
        ...
        for Index = 1, #Lower do
            Hash = (Hash + string.byte(Lower, Index) * Index) % 2147483647
        end

        return Palette[(Hash % #Palette) + 1]
    end

    local function RegisterElement(Element, TypeName, TabKey, RawTitle, RawDesc, Flag, DefaultValue, ValueState)
        if not Element then
            return Element
        end
        ...
        for _, TypeName in ipairs({ "Paragraph", "Button", "Toggle", "Slider", "Dropdown", "Input", "Keybind", "Colorpicker", "Section" }) do
            ...
            if typeof(Original) == "function" then
                Tab[TypeName] = function(Self, Options)
                    ...
                    local ValueState
                    if FlagTypes[TypeName] and New.Flag then
                        local InitialValue = New.Value
                        if TypeName == "Slider" and typeof(InitialValue) == "table" then
                            InitialValue = InitialValue.Default
                        elseif typeof(InitialValue) == "table" then
                            InitialValue = InitialValue.Value or InitialValue.Title or InitialValue[1]
                        end

                        ValueState = { Value = InitialValue }

                        if typeof(New.Callback) == "function" then
                            local Callback = New.Callback
                            New.Callback = function(Value, ...)
                                local TrackedValue = Value
                                if typeof(TrackedValue) == "table" then
                                    TrackedValue = TrackedValue.Value or TrackedValue.Title or TrackedValue[1]
                                end
                                ValueState.Value = TrackedValue
                    ...
                    RegisterElement(Element, TypeName, TabKey, RawTitle, RawDesc, New.Flag, New.Value, ValueState)
    ...
    --==================================================
    -- HUMAN-EDITABLE SETTINGS.JSON
    --==================================================
    do
        local SettingsFile = {
            Path = "MirrorsHub/MM2/settings.json",
            SchemaVersion = 1
        }

        Mirrors.SettingsFile = SettingsFile
        ...
        local function EnsureFolders()
            return EnsureFolder("MirrorsHub") and EnsureFolder("MirrorsHub/MM2")
        end
        ...
        local function Defaults()
            return {
                _schemaVersion = SettingsFile.SchemaVersion,
                _note = "Edit this file before starting the hub, or use Reload settings.json in Config. settings.json is applied after WindUI auto-load configs.",
                autoApply = true,
                interface = {
                    language = State.Language,
                    theme = State.Theme,
        ...
        local function Pretty(Data)
            local Interface = Data.interface or {}
            local Main = Data.main or {}
            local ESP = Data.esp or {}
            local Misc = Data.misc or {}

            return table.concat({
                "{",
                "  \"_schemaVersion\": " .. tostring(SettingsFile.SchemaVersion) .. ",",
                "  \"_note\": " .. Json(Data._note or Defaults()._note) .. ",",
                "  \"autoApply\": " .. tostring(Data.autoApply ~= false) .. ",",
                "  \"interface\": {",
                "    \"language\": " .. Json(Interface.language or State.Language) .. ",",
                "    \"theme\": " .. Json(Interface.theme or State.Theme) .. ",",
                "    \"notifications\": " .. tostring(Interface.notifications ~= false),
                "  },",
                "  \"main\": {",
                "    \"coinFarm\": " .. tostring(Main.coinFarm == true) .. ",",
                "    \"aimAssist\": " .. tostring(Main.aimAssist == true) .. ",",
                "    \"aimPart\": " .. Json(Main.aimPart or "Head") .. ",",
                "    \"aimSmoothness\": " .. tostring(tonumber(Main.aimSmoothness) or 18) .. ",",
                "    \"aimFOV\": " .. tostring(tonumber(Main.aimFOV) or 250),
                "  },",
                "  \"esp\": {",
                "    \"enabled\": " .. tostring(ESP.enabled == true) .. ",",
                "    \"mode\": " .. Json(ESP.mode or "Highlight") .. ",",
                "    \"box\": " .. tostring(ESP.box ~= false) .. ",",
                "    \"name\": " .. tostring(ESP.name ~= false) .. ",",
                "    \"role\": " .. tostring(ESP.role ~= false) .. ",",
                "    \"distance\": " .. tostring(ESP.distance ~= false) .. ",",
                "    \"tracer\": " .. tostring(ESP.tracer == true) .. ",",
                "    \"tracerOrigin\": " .. Json(ESP.tracerOrigin or "Bottom") .. ",",
                "    \"maxDistance\": " .. tostring(tonumber(ESP.maxDistance) or 1000) .. ",",
                "    \"drawingThickness\": " .. tostring(tonumber(ESP.drawingThickness) or 2) .. ",",
                "    \"textSize\": " .. tostring(tonumber(ESP.textSize) or 13) .. ",",
                "    \"highlightTransparency\": " .. tostring(tonumber(ESP.highlightTransparency) or 70) .. ",",
                "    \"murderer\": " .. tostring(ESP.murderer ~= false) .. ",",
                "    \"sheriff\": " .. tostring(ESP.sheriff ~= false) .. ",",
                "    \"innocent\": " .. tostring(ESP.innocent ~= false),
                "  },",
                "  \"misc\": {",
                "    \"antiAFK\": " .. tostring(Misc.antiAFK == true) .. ",",
                "    \"antiVoid\": " .. tostring(Misc.antiVoid == true) .. ",",
                "    \"antiFling\": " .. tostring(Misc.antiFling == true) .. ",",
                "    \"noClip\": " .. tostring(Misc.noClip == true) .. ",",
                "    \"walkSpeedEnabled\": " .. tostring(Misc.walkSpeedEnabled == true) .. ",",
                "    \"walkSpeed\": " .. tostring(tonumber(Misc.walkSpeed) or 32) .. ",",
                "    \"jumpPowerEnabled\": " .. tostring(Misc.jumpPowerEnabled == true) .. ",",
                "    \"jumpPower\": " .. tostring(tonumber(Misc.jumpPower) or 75) .. ",",
                "    \"fpsBoost\": " .. tostring(Misc.fpsBoost == true) .. ",",
                "    \"removeFog\": " .. tostring(Misc.removeFog == true) .. ",",
                "    \"fullbright\": " .. tostring(Misc.fullbright == true),
                "  }",
                "}"
            }, "\n")
        end
        ...
        local function Write(Data)
            if typeof(writefile) ~= "function" or not EnsureFolders() then
                return false, "File access is not supported by this executor."
            end

            local Success, ErrorMessage = pcall(function()
                writefile(SettingsFile.Path, Pretty(Data))
            end)
        ...
        function SettingsFile.Ensure()
            if typeof(isfile) ~= "function" then
                return false, "File access is not supported by this executor."
            end

            if isfile(SettingsFile.Path) then
                return true
            end
        ...
        function SettingsFile.Reset()
            return Write(Defaults())
        end

        function SettingsFile.Read()
            if typeof(readfile) ~= "function" or typeof(isfile) ~= "function" then
                return nil, "File access is not supported by this executor."
            end

            if not isfile(SettingsFile.Path) then
                local Created, ErrorMessage = SettingsFile.Ensure()
                if not Created then
                    return nil, ErrorMessage
                end
            end

            local ReadOK, Raw = pcall(readfile, SettingsFile.Path)
            if not ReadOK or type(Raw) ~= "string" then
                return nil, "Could not read settings.json."
            end

            local DecodeOK, Data = pcall(HttpService.JSONDecode, HttpService, Raw)
            if not DecodeOK or typeof(Data) ~= "table" then
                return nil, "settings.json contains invalid JSON."
            end

            return Data
        end

        local function FindByFlag(Flag)
        ...
        local function NormalizeDropdown(Value)
            if typeof(Value) == "table" then
                return Value.Value or Value.Title or Value[1]
            end
            return Value
        end

        local function CurrentValue(Flag, Fallback)
            local Entry = FindByFlag(Flag)
            if not Entry or not Entry.Element then
                return Fallback
            end

            if Entry.ValueState and Entry.ValueState.Value ~= nil then
                local Tracked = NormalizeDropdown(Entry.ValueState.Value)
                if typeof(Tracked) == "boolean" or typeof(Tracked) == "number" or typeof(Tracked) == "string" then
                    return Tracked
                end
            end
        ...
        local function ApplyFlag(Flag, Value)
            ...
            if Entry.Type == "Dropdown" and typeof(Element.Select) == "function" then
            ...
            elseif typeof(Element.Select) == "function" then
                Success = pcall(function()
                    Element:Select(Value)
                end)
            end
        ...
        local function Allowed(Value, Options, Fallback)
        ...
        function SettingsFile.Apply(Data, Silent, Force)
            if typeof(Data) ~= "table" then
                return false, "Invalid settings data."
            end

            if Data.autoApply == false and not Force then
                return true, "Auto apply disabled."
            end

            local Interface = typeof(Data.interface) == "table" and Data.interface or {}
            local Main = typeof(Data.main) == "table" and Data.main or {}
            local ESP = typeof(Data.esp) == "table" and Data.esp or {}
            local Misc = typeof(Data.misc) == "table" and Data.misc or {}

            State.ConfigApplying = true

            local DesiredLanguage = Allowed(Interface.language, { "English", "Português", "Español" }, State.Language)
            local DesiredTheme = Allowed(Interface.theme, ThemeOptions, State.Theme)
            local DesiredNotifications = State.Notifications
            if typeof(Interface.notifications) == "boolean" then
                DesiredNotifications = Interface.notifications
            end

            -- Prevent startup/reload spam while callbacks apply the file.
            State.Notifications = false
            Mirrors.SetLanguage(DesiredLanguage, true)
            Mirrors.SetTheme(DesiredTheme, true)

            ApplyFlag("Config_Dropdown_Language", DesiredLanguage)
            ApplyFlag("Config_Dropdown_Theme", DesiredTheme)

            if typeof(Main.coinFarm) == "boolean" then ApplyFlag("Main_Toggle_CoinFarm", Main.coinFarm) end
            if typeof(Main.aimAssist) == "boolean" then ApplyFlag("Main_Toggle_AimAssist", Main.aimAssist) end
            ApplyFlag("Main_Dropdown_AimPart", Allowed(Main.aimPart, { "Head", "HumanoidRootPart", "UpperTorso", "LowerTorso", "Torso" }, "Head"))
            ApplyFlag("Main_Slider_AimSmoothness", Number(Main.aimSmoothness, 1, 100, 18))
            ApplyFlag("Main_Slider_AimFOV", Number(Main.aimFOV, 50, 600, 250))

            if typeof(ESP.enabled) == "boolean" then ApplyFlag("ESP_Toggle_EnableESP", ESP.enabled) end
            ApplyFlag("ESP_Dropdown_ESPMode", Allowed(ESP.mode, { "Highlight", "Drawing", "Both" }, "Highlight"))
            if typeof(ESP.box) == "boolean" then ApplyFlag("ESP_Toggle_Box", ESP.box) end
            if typeof(ESP.name) == "boolean" then ApplyFlag("ESP_Toggle_Name", ESP.name) end
            if typeof(ESP.role) == "boolean" then ApplyFlag("ESP_Toggle_Role", ESP.role) end
            if typeof(ESP.distance) == "boolean" then ApplyFlag("ESP_Toggle_Distance", ESP.distance) end
            if typeof(ESP.tracer) == "boolean" then ApplyFlag("ESP_Toggle_Tracer", ESP.tracer) end
            ApplyFlag("ESP_Dropdown_TracerOrigin", Allowed(ESP.tracerOrigin, { "Bottom", "Center", "Top" }, "Bottom"))
            ApplyFlag("ESP_Slider_MaxDistance", Number(ESP.maxDistance, 50, 2500, 1000))
            ApplyFlag("ESP_Slider_DrawingThickness", Number(ESP.drawingThickness, 1, 4, 2))
            ApplyFlag("ESP_Slider_TextSize", Number(ESP.textSize, 10, 20, 13))
            ApplyFlag("ESP_Slider_HighlightTransparency", Number(ESP.highlightTransparency, 0, 100, 70))
            if typeof(ESP.murderer) == "boolean" then ApplyFlag("ESP_Toggle_MurdererESP", ESP.murderer) end
            if typeof(ESP.sheriff) == "boolean" then ApplyFlag("ESP_Toggle_SheriffESP", ESP.sheriff) end
            if typeof(ESP.innocent) == "boolean" then ApplyFlag("ESP_Toggle_InnocentESP", ESP.innocent) end

            if typeof(Misc.antiAFK) == "boolean" then ApplyFlag("Misc_Toggle_AntiAFK", Misc.antiAFK) end
            if typeof(Misc.antiVoid) == "boolean" then ApplyFlag("Misc_Toggle_AntiVoid", Misc.antiVoid) end
            if typeof(Misc.antiFling) == "boolean" then ApplyFlag("Misc_Toggle_AntiFling", Misc.antiFling) end
            if typeof(Misc.noClip) == "boolean" then ApplyFlag("Misc_Toggle_NoClip", Misc.noClip) end
            if typeof(Misc.walkSpeedEnabled) == "boolean" then ApplyFlag("Misc_Toggle_WalkSpeed", Misc.walkSpeedEnabled) end
            ApplyFlag("Misc_Slider_WalkSpeedValue", Number(Misc.walkSpeed, 16, 100, 32))
            if typeof(Misc.jumpPowerEnabled) == "boolean" then ApplyFlag("Misc_Toggle_JumpPower", Misc.jumpPowerEnabled) end
            ApplyFlag("Misc_Slider_JumpPowerValue", Number(Misc.jumpPower, 50, 150, 75))
            if typeof(Misc.fpsBoost) == "boolean" then ApplyFlag("Misc_Toggle_FPSBoost", Misc.fpsBoost) end
            if typeof(Misc.removeFog) == "boolean" then ApplyFlag("Misc_Toggle_RemoveFog", Misc.removeFog) end
            if typeof(Misc.fullbright) == "boolean" then ApplyFlag("Misc_Toggle_Fullbright", Misc.fullbright) end

            Mirrors.SetNotifications(DesiredNotifications, true)
            ApplyFlag("Config_Toggle_Notifications", DesiredNotifications)

            State.ConfigApplying = false

            if not Silent then
                Notify("settings.json", "Settings applied successfully.", "solar:check-square-bold", 3)
            end

            return true
        end

        function SettingsFile.ApplyFromDisk(Silent, Force)
            local Data, ErrorMessage = SettingsFile.Read()
            if not Data then
                if not Silent then
                    Notify("settings.json", ErrorMessage or "Unable to read settings.json.", "x", 4)
                end
                return false, ErrorMessage
            end

            return SettingsFile.Apply(Data, Silent, Force)
        end

        function SettingsFile.SaveCurrent()
            local Data = Defaults()

            Data.interface.language = CurrentValue("Config_Dropdown_Language", State.Language)
            Data.interface.theme = CurrentValue("Config_Dropdown_Theme", State.Theme)
            Data.interface.notifications = CurrentValue("Config_Toggle_Notifications", State.Notifications)

            Data.main.coinFarm = CurrentValue("Main_Toggle_CoinFarm", false)
            Data.main.aimAssist = CurrentValue("Main_Toggle_AimAssist", false)
            Data.main.aimPart = CurrentValue("Main_Dropdown_AimPart", "Head")
            Data.main.aimSmoothness = CurrentValue("Main_Slider_AimSmoothness", 18)
            Data.main.aimFOV = CurrentValue("Main_Slider_AimFOV", 250)

            Data.esp.enabled = CurrentValue("ESP_Toggle_EnableESP", false)
            Data.esp.mode = CurrentValue("ESP_Dropdown_ESPMode", "Highlight")
            Data.esp.box = CurrentValue("ESP_Toggle_Box", true)
            Data.esp.name = CurrentValue("ESP_Toggle_Name", true)
            Data.esp.role = CurrentValue("ESP_Toggle_Role", true)
            Data.esp.distance = CurrentValue("ESP_Toggle_Distance", true)
            Data.esp.tracer = CurrentValue("ESP_Toggle_Tracer", false)
            Data.esp.tracerOrigin = CurrentValue("ESP_Dropdown_TracerOrigin", "Bottom")
            Data.esp.maxDistance = CurrentValue("ESP_Slider_MaxDistance", 1000)
            Data.esp.drawingThickness = CurrentValue("ESP_Slider_DrawingThickness", 2)
            Data.esp.textSize = CurrentValue("ESP_Slider_TextSize", 13)
            Data.esp.highlightTransparency = CurrentValue("ESP_Slider_HighlightTransparency", 70)
            Data.esp.murderer = CurrentValue("ESP_Toggle_MurdererESP", true)
            Data.esp.sheriff = CurrentValue("ESP_Toggle_SheriffESP", true)
            Data.esp.innocent = CurrentValue("ESP_Toggle_InnocentESP", true)

            Data.misc.antiAFK = CurrentValue("Misc_Toggle_AntiAFK", false)
            Data.misc.antiVoid = CurrentValue("Misc_Toggle_AntiVoid", false)
            Data.misc.antiFling = CurrentValue("Misc_Toggle_AntiFling", false)
            Data.misc.noClip = CurrentValue("Misc_Toggle_NoClip", false)
            Data.misc.walkSpeedEnabled = CurrentValue("Misc_Toggle_WalkSpeed", false)
            Data.misc.walkSpeed = CurrentValue("Misc_Slider_WalkSpeedValue", 32)
            Data.misc.jumpPowerEnabled = CurrentValue("Misc_Toggle_JumpPower", false)
            Data.misc.jumpPower = CurrentValue("Misc_Slider_JumpPowerValue", 75)
            Data.misc.fpsBoost = CurrentValue("Misc_Toggle_FPSBoost", false)
            Data.misc.removeFog = CurrentValue("Misc_Toggle_RemoveFog", false)
            Data.misc.fullbright = CurrentValue("Misc_Toggle_Fullbright", false)
    ...
    local UtilitiesSection = Window:Section({ Title = Translate("UTILITIES"), Opened = true })
    local SettingsSection = Window:Section({ Title = Translate("SETTINGS"), Opened = true })
    ...
    local Tabs = {
        Info = CreateTab(General, "Info", "Server & round", "info"),
        Main = CreateTab(General, "Main", "Automation & players", "house"),
        ESP = CreateTab(General, "ESP", "Role visuals", "eye"),
        Murder = CreateTab(RolesSection, "Murder", "Role actions", "skull"),
        Sheriff = CreateTab(RolesSection, "Sheriff", "Role actions", "shield"),
        Innocent = CreateTab(RolesSection, "Innocent", "Role actions", "user"),
        Shortcuts = CreateTab(UtilitiesSection, "Shortcuts", "Quick panel", "zap"),
        Misc = CreateTab(UtilitiesSection, "Misc", "Server & player", "wrench"),
        Config = CreateTab(SettingsSection, "Config", "Hub & files", "settings")
    }
    ...
    local function StyleOpenButton()
        local Button = FindButton("Open Mirrors Hub - MM2")
        if not Button then
            return
        end
...
local function GetESPColor(Role)
    local Normalized = NormalizeESPRole(Role)
    if not Normalized then
        return nil
    end

    return ESPColors[Role] or ESPColors[Normalized]
end
...
local function GetESPCharacter(Player)
    ...
    or Player:GetAttribute("Alive") == false then
        return
    end
...
local function GetCharacterScreenBounds(
    ...
    if RootPoint.Z <= 0.25 then
        return
    end

    -- Only body parts are used here. Tools/accessories are intentionally ignored;
    -- MM2's equipped knife can otherwise make Character:GetBoundingBox() enormous.
    local Head = Character:FindFirstChild("Head")
    local TopY

    if Head and Head:IsA("BasePart") then
        TopY = Head.Position.Y + math.max(Head.Size.Y * 0.55, 0.45)
    else
        TopY = Root.Position.Y + 3.1
    end
    ...
    if not FoundLowerBody then
        BottomY = Root.Position.Y - math.max(Humanoid.HipHeight + 1.35, 2.75)
    end

    local WorldHeight = TopY - BottomY
    if WorldHeight < 2 or WorldHeight > 12 then
        return
    end
    ...
    if Top.Z <= 0.25 or Bottom.Z <= 0.25 then
        return
    end

    local Height = math.abs(Bottom.Y - Top.Y)
    local MaxHeight = math.min(Viewport.Y * 0.92, 720)
    if Height < 8 or Height > MaxHeight then
        return
    end

    local CenterX = (Top.X + Bottom.X + RootPoint.X) / 3
    local MinY = math.min(Top.Y, Bottom.Y) - 2
    local MaxY = math.max(Top.Y, Bottom.Y) + 2

    -- A humanoid box is roughly half as wide as it is tall on screen.
    -- Clamp it so a single bad projection frame can never cover the screen.
    local Width = math.clamp(
        Height * 0.52,
        14,
        math.min(Viewport.X * 0.34, 260)
    )
...
local function SetESPEnabled(Value)
    ESPSettings.Enabled = Value == true
    ESPAccumulator = 0
...
SectionLine(ConfigTab, "Settings File")

ConfigTab:Paragraph({
    Title = "settings.json",
    Desc = "Path: MirrorsHub/MM2/settings.json\nEdit this file on your executor workspace. It is applied after auto-load configs."
})

ConfigTab:Button({
    Title = "Reload settings.json",
    Icon = "refresh-cw",
    Callback = function()
        local Success, ErrorMessage = Mirrors.SettingsFile.ApplyFromDisk(false, true)
        if not Success then
            Mirrors.Notify("settings.json", ErrorMessage or "Unable to apply settings.json.", "x", 4)
        end
    end
})

ConfigTab:Button({
    Title = "Save Current settings.json",
    Icon = "save",
    Callback = function()
        local Success, ErrorMessage = Mirrors.SettingsFile.SaveCurrent()
        if Success then
            Mirrors.Notify("settings.json", "Current settings saved.", "solar:check-square-bold", 3)
        else
            Mirrors.Notify("settings.json", ErrorMessage or "Unable to save settings.json.", "x", 4)
        end
    end
})

ConfigTab:Button({
    Title = "Reset settings.json",
    Icon = "rotate-ccw",
    Callback = function()
        local Success, ErrorMessage = Mirrors.SettingsFile.Reset()
        if Success then
            Mirrors.Notify("settings.json", "Default settings file restored.", "solar:check-square-bold", 3)
        else
            Mirrors.Notify("settings.json", ErrorMessage or "Unable to reset settings.json.", "x", 4)
        end
    end
})
...
ConfigTab:Button({
    ...
    Callback = function()
        ...
        if Clipboard and pcall(Clipboard, Mirrors.SettingsFile.Path) then
            Mirrors.Notify("settings.json", "Path copied to clipboard.", "copy", 2)
        else
            Mirrors.Notify("settings.json", Mirrors.SettingsFile.Path, "info", 4)
        end
    end
})
...
local SettingsCreated, SettingsCreateError = Mirrors.SettingsFile.Ensure()
if not SettingsCreated and SettingsCreateError then
    Mirrors.Notify("settings.json", SettingsCreateError, "x", 4)
end
...
-- settings.json intentionally has final startup priority so manual edits are
-- deterministic even when a WindUI config is also set to auto-load.
task.delay(SavedAutoLoad and 1.15 or 0.35, function()
    if Mirrors.Runtime.Alive then
        local Success, ErrorMessage = Mirrors.SettingsFile.ApplyFromDisk(true, false)
        if not Success and ErrorMessage then
            Mirrors.Notify("settings.json", ErrorMessage, "x", 4)
        end
    end
end)
