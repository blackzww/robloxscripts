...
["Your executor does not support file configs."] = "Tu executor no soporta configuraciones en archivo.",
            ...
            if typeof(Original) == "function" then
                Tab[TypeName] = function(Self, Options)
                    ...
                    if TypeName == "Button" then
                        local Color = ButtonVisual(RawTitle)
                        New.Color = New.Color or Color
                        if New.Icon then
                            New.IconAlign = New.IconAlign or "Left"
                        end
                        New.Justify = New.Justify or (RawDesc and "Left" or "Center")
                    end
    ...
    local InitialTheme = State.Theme
    pcall(function()
    ...
    local function ClearSelectedGlow()
        for _, Button in pairs(TabButtons) do
            ...
            RemoveNamed(Button, "MirrorsV52TabAccent")
        end
    end
    ...
    local function ApplySelectedGlow(Key, Button)
        ...
        local Edge = AddWhiteGlow(Button, "MirrorsV51SelectedEdge", 1.25, 0.20)
        local Glow = AddWhiteGlow(Button, "MirrorsV51SelectedGlow", 3.2, 0.72)
        if Edge then Edge.Color = C.White end
        if Glow then Glow.Color = C.White end

        if not Button:FindFirstChild("MirrorsV52TabAccent") then
            local Accent = Instance.new("Frame")
            Accent.Name = "MirrorsV52TabAccent"
            ...
            AddWhiteGlow(Accent, "MirrorsV52TabAccentGlow", 2.2, 0.65)
        end
    end
    ...
    local function StyleTab(Key)
        ...
        if IconContainer then
            ...
            RemoveNamed(IconContainer, "MirrorsV51IconEdge")
            RemoveNamed(IconContainer, "MirrorsV51IconGlow")
        ...
        for _, Entry in ipairs(Registry) do
            if Entry.Type ~= "Tab" and typeof(Entry.Title) == "string" then
                ...
                if Frame and not Frame:FindFirstChild("MirrorsV51ElementGlow") then
                    local Glow = AddWhiteGlow(Frame, "MirrorsV51ElementGlow", 1.6, 0.92)
                    local Bound = Instance.new("BoolValue")
                    Bound.Name = "MirrorsV51HoverBound"
                    Bound.Parent = Frame
                    TrackConnection(Frame.MouseEnter:Connect(function()
                        if Glow and Glow.Parent then
                            TweenService:Create(Glow, TweenInfo.new(0.16), { Transparency = 0.76 }):Play()
                        end
                    end))
                    TrackConnection(Frame.MouseLeave:Connect(function()
                        if Glow and Glow.Parent then
                            TweenService:Create(Glow, TweenInfo.new(0.20), { Transparency = 0.92 }):Play()
                        end
                    end))
                end
                ...
                if Entry.Type == "Button" then
                    ...
                    if Button and not Button:FindFirstChild("MirrorsV51ButtonScale") then
                        ...
                        Scale.Scale = 1
                        Scale.Parent = Button
                        local Glow = AddWhiteGlow(Button, "MirrorsV51ButtonGlow", 2.8, 0.78)
                        TrackConnection(Button.Activated:Connect(function()
                            TweenService:Create(Scale, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1.055 }):Play()
                            if Glow then
                                TweenService:Create(Glow, TweenInfo.new(0.10), { Transparency = 0.42 }):Play()
                            end
                            task.delay(0.14, function()
                                if Scale.Parent then
                                    TweenService:Create(Scale, TweenInfo.new(0.20, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
                                    if Glow and Glow.Parent then
                                        TweenService:Create(Glow, TweenInfo.new(0.28), { Transparency = 0.78 }):Play()
                                    end
                                end
                            end)
                        end))
    ...
    local function StyleRoundedControls()
        ...
        for _, Object in ipairs(Root:GetDescendants()) do
            if Object:IsA("GuiButton") then
                local Size = Object.AbsoluteSize
                if Size.X >= 32 and Size.Y >= 28 and Size.Y <= 96 then
                    local Radius = math.clamp(math.floor(Size.Y * 0.30), 9, 16)
                    EnsureCorner(Object, Radius)

                    local Parent = Object.Parent
                    for _ = 1, 2 do
                        if Parent and Parent:IsA("Frame") then
                            local PSize = Parent.AbsoluteSize
                            if math.abs(PSize.X - Size.X) <= 8 and math.abs(PSize.Y - Size.Y) <= 8 then
                                EnsureCorner(Parent, Radius)
                            end
                        end
