...
-- ============================================
-- RESOURCE MANAGER (Base class for all)
-- ============================================
...
function ResourceManager:addHighlight(key, highlight)
    local bucket = self.highlights[key]
    if not bucket then
        bucket = {}
        self.highlights[key] = bucket
    end
    table.insert(bucket, highlight)
end

function ResourceManager:hasKey(key)
    return self.highlights[key] ~= nil or self.connections[key] ~= nil or 
           self.instances[key] ~= nil or self.threads[key] ~= nil
end
...
function ResourceManager:cleanupKey(key)
    ...
    if self.highlights[key] then
        for _, highlight in pairs(self.highlights[key]) do
...
function ResourceManager:cleanupAll()
    local keys = {}
    for key in pairs(self.connections) do keys[key] = true end
    for key in pairs(self.threads) do keys[key] = true end
    for key in pairs(self.instances) do keys[key] = true end
    for key in pairs(self.highlights) do keys[key] = true end
...
-- ============================================
-- SUPER ESP (Player ESP)
-- ============================================
local SuperESPConfig = {
    Colors = {
...
function SuperESP:getPlayerColor(player)
    if self:isPlayerBeast(player) then
        return SuperESPConfig.Colors.Beast
    end
    return SuperESPConfig.Colors.Survivor
end

function SuperESP:createVisuals(character, rootPart)
    local key = "visuals_" .. character.Name
    
    local existingFolder = character:FindFirstChild(SuperESPConfig.FolderName)
    if existingFolder then
        existingFolder:Destroy()
    end
    
    local folder = Instance.new("Folder")
    folder.Name = SuperESPConfig.FolderName
    folder.Parent = character
    self.resources:addInstance(key, folder)
    
    local highlight = Instance.new("Highlight")
    highlight.Adornee = character
    highlight.FillTransparency = SuperESPConfig.FillTransparency
    highlight.OutlineTransparency = SuperESPConfig.OutlineTransparency
    highlight.Parent = folder
    self.resources:addInstance(key, highlight)
    
    local billboard = Instance.new("BillboardGui")
    billboard.Adornee = rootPart
    billboard.Size = SuperESPConfig.BillboardSize
    billboard.StudsOffset = SuperESPConfig.BillboardOffset
    ...
    label.Font = SuperESPConfig.Font
    label.TextSize = SuperESPConfig.TextSize
...
function SuperESP:startUpdateLoop(player, character, rootPart, visuals)
    ...
    local thread = task.spawn(function()
        while self.enabled do
            ...
            task.wait(SuperESPConfig.UpdateInterval)
        end
    end)
...
function SuperESP:setupPlayer(player)
    ...
    local function onCharacterAdded(character)
        local rootPart = character:WaitForChild("HumanoidRootPart", SuperESPConfig.HumanoidRootPartTimeout)
        if not rootPart then
            warn("[SuperESP] HumanoidRootPart not found for " .. player.Name)
            return
        end
...
function SuperESP:disable()
    ...
    for _, player in pairs(self.getPlayers()) do
        ...
        if character then
            local folder = character:FindFirstChild(SuperESPConfig.FolderName)
            if folder then
                folder:Destroy()
            end
        end
...
function HighlightFactory.new(config)
    ...
    self._config = config or DoorESPConfig.HighlightDefaults
    return self
end
...
function DoorStateReader.new(stateConfig)
    ...
    self._config = stateConfig or DoorESPConfig.StateValues
    return self
end
...
function UpdateScheduler.new(resourceManager, updateInterval)
    ...
    self._interval = updateInterval or DoorESPConfig.Timing.UpdateInterval
    self._tasks = {}
    return self
end
...
function DoorESP:_initializeDoor(door, doorTypeConfig)
    ...
    self._scheduler:registerTask(doorKey, function()
        ...
        local color = self._stateReader:getColor(door, doorTypeConfig, DoorESPConfig.Colors)
        if color then
...
function DoorESP:enable()
    ...
    for _, doorTypeConfig in pairs(DoorESPConfig.DoorTypes) do
...
local DoorESPToggle = Esp:Toggle({
    Title = "Door ESP",
...
function ComputerDiscoveryService:discover()
	...
	local computers = {}
	for _, descendant in pairs(self._workspace:GetDescendants()) do
		if descendant.Name == ComputerESPConfig.ComputerName then
...
function ComputerStateReader.new(brickColorConfig)
	...
	self._config = brickColorConfig or ComputerESPConfig.BrickColors
	return self
end
...
function ComputerESP:_updateComputerColor(computer, highlight)
	...
	local color = self._stateReader:getColor(computer, ComputerESPConfig.Colors)
	if color then
		highlight.FillColor = color
		highlight.OutlineColor = color
	end
...
local ComputerESPToggle = Esp:Toggle({
	Title = "Computer ESP",
