...
-- Production Final v1.6.1 - register-safe refactor of Universal 1.5
--==================================================
...
previousRuntime=ENV.MirrorsHubUniversalRuntime
...
HUB_FOLDER="MirrorsHub"
CONFIG_FOLDER="MirrorsHub/config"
LANG_FILE=CONFIG_FOLDER.."/language.json"
booting=true
...
function L(k)local s=TXT[Lang]or TXT.English;return s[k]or TXT.English[k]or k end
loadLang()
...
EXTRA_TXT={
    English={
        ...
        HomeDesc="Universal script hub.\nVersion: 1.6.1 Production Final\nCreated by blackzw.mp3",StatusDesc="Build: 1.6.1 Production Final.",ConfigPath="Folder: MirrorsHub/config\nProfile-based WindUI ConfigManager"
    },
    Portuguese={
        ...
        HomeDesc="Hub universal de scripts.\nVersão: 1.6.1 Production Final\nCriado por blackzw.mp3",StatusDesc="Build: 1.6.1 Production Final.",ConfigPath="Pasta: MirrorsHub/config\nConfigManager WindUI por perfil"
    },
    Spanish={
        ...
        HomeDesc="Hub universal de scripts.\nVersión: 1.6.1 Production Final\nCreado por blackzw.mp3",StatusDesc="Build: 1.6.1 Production Final.",ConfigPath="Carpeta: MirrorsHub/config\nConfigManager WindUI por perfil"
    }
}
...
STARTUP_FILE=CONFIG_FOLDER.."/startup.json"
...
startupMeta=readStartupMeta()

Aim={Enabled=false,FOV=140,Smoothness=.09,Strength=1,TargetPart="Head",TargetSwitchDelay=.25,TeamCheck=false,VisibleCheck=false,FOVVisible=false,FOVColor=Color3.fromRGB(134,0,212),Crosshair=false,CrosshairSize=8}
EspC={Enabled=false,Color=Color3.fromRGB(0,255,255),ShowNames=false,ShowHealth=false,ShowDistance=false,ShowLines=false,TeamCheck=false,Fill=false,FillTransparency=.75,OutlineTransparency=0,TextSize=13,MaxDistance=5000,TeamColor=false,Rainbow=false,TracerOrigin="Bottom"}
MiscC={SpeedEnabled=false,WalkSpeed=16,JumpEnabled=false,JumpPower=50,Noclip=false,InfiniteJump=false,AntiAFK=false,AntiVoid=true,Fullbright=false,NoFog=false,Performance=false}
HitC={Enabled=false,Size=10,MaxSize=200,Transparency=.65,Color=Color3.fromRGB(134,0,212),TeamCheck=false}
OpC={God100=false,GodInf=false,AntiFling=false,TouchFling=false,FlingPower=10000,FlingPulse=.1,FreezeSelected=false,FreezeAll=false}
TrollC={FakeLag=false,FakeLagRate=.22,FakeLagHold=.08,FakeDeath=false,FakeDeathCooldown=1.2,Orbit=false,OrbitMode="Circle",OrbitRadius=7,OrbitSpeed=8,OrbitHeight=4,Spin=false,SpinSpeed=35,HeadSit=false,Annoy=false,AnnoyMode="Circle",AnnoySpeed=.05,Ghost=true}

OPSelectedPlayer="None"
TrollSelectedPlayer="None"
JobInput=""
LastTeleportCF=nil
ProfileName=tostring(startupMeta.ProfileName or "default")
AutoSave=false
AutoLoadConfig=startupMeta.AutoLoadConfig~=false
NotifyOn=true
ThemeName="Mirrors Purple"
UIScaleValue=1
BackgroundTransparencyValue=0
booting=true
ConfigDirty=false
...
ENABLE_LEGACY_EXTERNAL_APIS=false
LEGACY_TELEMETRY_URL="https://mirrorskey-system.vercel.app/api/log-use"
LEGACY_CHAT_URL="https://mirrorskey-system.vercel.app/api/chat"
...
KeyInfo={Status="Unknown",Provider="Unknown",ExpiresAt=nil}
KEY_API_URL="https://mirrorshub-key.vercel.app/api/key/validate"
KeyMessages={
    INVALID_KEY="Invalid key.",INVALID_HWID="Unable to identify this device.",HWID_MISMATCH="This key is linked to another device.",
    INVALID_ROBLOX_USER="Invalid Roblox user.",ROBLOX_USER_NOT_FOUND="Roblox user not found.",ROBLOX_USER_MISMATCH="This key is linked to another Roblox account.",
...
function ValidateKey(key)
    ...
    local statusCode=tonumber(res.StatusCode or res.Status or res.status_code or 0) or 0
    ...
    KeyInfo.Status=code
    warn("[MIRRORS] "..tostring(KeyMessages[code] or code)..(statusCode>0 and (" [HTTP "..statusCode.."]") or ""))
    return false
end
...
okWind,windSource=pcall(function()return game:HttpGet(WINDUI_URL,true)end)
if not okWind or type(windSource)~="string" or #windSource<1000 then
    error("[MIRRORS] Failed to download WindUI: "..tostring(windSource))
end
windChunk,windCompileError=loadstring(windSource)
if type(windChunk)~="function" then
    error("[MIRRORS] WindUI compile failed: "..tostring(windCompileError))
end
okWindRuntime,WindUI=pcall(windChunk)
if not okWindRuntime or type(WindUI)~="table" then
    error("[MIRRORS] WindUI startup failed: "..tostring(WindUI))
end
...
function char(p)return (p or LP).Character end
function hum(p)local c=char(p);return c and c:FindFirstChildOfClass("Humanoid") end
function root(p)local c=char(p);return c and c:FindFirstChild("HumanoidRootPart") end
...
function getOPSelected()return getPlayerByName(OPSelectedPlayer) end
function getTrollSelected()return getPlayerByName(TrollSelectedPlayer) end

Frame={Camera=Camera,Center=Vector2.zero,Now=0}
...
HasDrawing=Drawing and Drawing.new
FOVCircle,CrossA,CrossB=nil,nil,nil
if HasDrawing then
...
function clearESP()for p in pairs(ESPObjects)do removeESP(p)end end
function refreshESP()clearESP();if EspC.Enabled then for p in pairs(TrackedPlayers)do ensureESPBase(p)end end end
function updateESP()
...
function attachNoclipCharacter()
...
function freezePlayer(p,reason)
...
function unfreezeReason(p,reason)
...
function unfreezeAll()for p in pairs(Frozen)do unfreezePlayer(p)end end
function setFreezeSelected(on)
    on=on and true or false
...
LagState={Timer=0,Holding=false,Character=nil,Root=nil,Anchored=nil}
function releaseFakeLagHold()
    local s=LagState
...
function setControl(flag,value)
...
function applyAntiFlingToPlayer(p)
...
function teleportBack()local r=root();if r and LastTeleportCF then r.CFrame=LastTeleportCF end end

Performance={Enabled=false,Backup=setmetatable({}, {__mode="k"}),Terrain=nil}
function protectedPerformanceObject(v)
...
function hop()local s=getServer(false);if s then TP:TeleportToPlaceInstance(game.PlaceId,s.id,LP)else notify(L("NoServer"),"x",3)end end
function smallServer()local s=getServer(true);if s then TP:TeleportToPlaceInstance(game.PlaceId,s.id,LP)else notify(L("NoServer"),"x",3)end end
function joinJob()if JobInput and #JobInput>5 then TP:TeleportToPlaceInstance(game.PlaceId,JobInput,LP)end end
function copyJob()if type(setclipboard)=="function" then setclipboard(game.JobId);notify(L("Copied"),"copy",2)else notify(game.JobId,"copy",5)end end
...
syncAllControls=nil
applyAllFeatureState=nil
function mergeTable(dst,src)if type(src)=="table" then for k,v in pairs(src)do dst[k]=v end end end
...
function loadActiveConfig(silent)
...
function registerControl(flag,control)if flag and control then Controls[flag]=control end;return control end
function section(tab,key)tab:Section({Title=L(key),Box=false,TextSize=17}) end
function para(tab,title,desc,color)return tab:Paragraph({Title=title,Desc=desc,Color=color or "Blue",Locked=false}) end
function btn(tab,key,cb)return tab:Button({Title=L(key),Desc="",Locked=false,Callback=cb}) end
function btnTitle(tab,title,cb)return tab:Button({Title=title,Desc="",Locked=false,Callback=cb}) end
function tog(tab,key,flag,val,cb)
...
function slid(tab,key,flag,min,max,def,step,cb)
...
function drop(tab,key,flag,vals,val,cb)
...
function col(tab,key,flag,def,cb)
...
function input(tab,key,flag,ph,cb)
...
function shortId(v)v=tostring(v or "");return #v>18 and (v:sub(1,8).."..."..v:sub(-6)) or v end
...
function serverAge()return workspace.DistributedGameTime and workspace.DistributedGameTime>0 and workspace.DistributedGameTime or os.clock()-ServerStart end
function playerPositionText()local r=root();return r and (math.floor(r.Position.X)..", "..math.floor(r.Position.Y)..", "..math.floor(r.Position.Z)) or "N/A" end
CurrentFPS=0
...
DISCORD_LINK="https://discord.gg/YZEg6FyRSF"
function copyDiscord()if type(setclipboard)=="function" then setclipboard(DISCORD_LINK);notify(L("Copied"),"copy",2)else notify(DISCORD_LINK,"copy",8)end end
function runExternal(src)
    notify("External script: EXTERNAL_SCRIPT_UNMANAGED","triangle-alert",3)
    local ok,raw=pcall(function()return game:HttpGet(src,true)end)
    if not ok or type(raw)~="string" then notify("Download failed: "..tostring(raw),"x",4)return end
    local fn,compileError=loadstring(raw)
    if type(fn)~="function" then notify("Compile failed: "..tostring(compileError),"x",5)return end
    local ran,runtimeError=pcall(fn)
    if not ran then notify("Runtime failed: "..tostring(runtimeError),"x",5)end
end
...
KeyParagraph=para(Tabs.Info,L("KeyInformation"),"Loading...","Blue")
...
OPPlayerDrop=drop(Tabs.OP,"SelectPlayer","universal_op_selectedplayer",playerNames(),OPSelectedPlayer,function(v)
...
TrollPlayerDrop=drop(Tabs.Troll,"SelectPlayer","universal_troll_selectedplayer",playerNames(),TrollSelectedPlayer,function(v)TrollSelectedPlayer=v or "None" end)
...
ConfigListDrop=drop(Tabs.Config,"ConfigList","universal_config_profile",configNames(),sanitizeProfileName(ProfileName),function(v)if v then ProfileName=tostring(v);saveStartupMeta()end end)
...
KeybindControl=Tabs.Config:Keybind({Title=L("ToggleKey"),Desc="",Flag="universal_ui_togglekey",Value="H",Callback=function(v)if v and Enum.KeyCode[v] and Window.SetToggleKey then Window:SetToggleKey(Enum.KeyCode[v])end end})
...
StatusParagraph=para(Tabs.Status,L("LiveStatus"),"Loading...","Green")
PlayerStatusParagraph=para(Tabs.Status,L("PlayerInfo"),"Loading...","Blue")
...
AIResponseParagraph=para(Tabs.AI,L("AIResponse"),L("AIAwaiting"),"Blue")
AskButton=nil
...
function onOtherCharacterAdded(p,c)
    task.defer(function()
...
function onLocalCharacterAdded(c)
    lastSafePosition=nil
    MovementSnapshot=nil
    FakeDeathState={Active=false,Character=nil,Humanoid=nil,Root=nil,PlatformStand=nil,RootJoint=nil,RootJointC0=nil}
    GodState=nil
...
function onLocalCharacterRemoving(c)
    if LP.Character~=c then return end
    lastSafePosition=nil
    Runtime:SetConnection("NoclipDescendant",nil)
    NoclipBackup=setmetatable({}, {__mode="k"})
...
antiVoidAcc,espAcc,hitAcc,statusAcc,reconcileAcc=0,0,0,0,0
