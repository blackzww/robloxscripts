...
local AITab=Window:Tab({Title=L("AskAI"),Icon="sparkles"})
AITab:Section({Title="MIRRORS AI"})
local PT,CD,LTime="",10,0
AITab:Input({Title=L("YourQuestion"),Desc=L("QuestionDesc"),Placeholder=L("QuestionPlaceholder"),Callback=function(t)PT=t end})
local Resp=AITab:Paragraph({Title=L("AIResponse"),Desc=L("AIAwaiting")})
local AskBtn
AskBtn=AITab:Button({Title=L("SendButton"),Desc=L("SendDesc"),Callback=function()
local now=os.clock()
if now-LTime<CD then Resp:SetDesc(string.format(L("CooldownText"),math.ceil(CD-(now-LTime)))) return end
if PT=="" or #PT<3 then Resp:SetDesc(L("InvalidQuestion")) return end
LTime=os.clock() Resp:SetDesc(L("Thinking"))
task.spawn(function()
for i=CD,1,-1 do AskBtn:SetTitle(L("SendButton").." ("..i.."s)") task.wait(1) end
AskBtn:SetTitle(L("SendButton"))
end)
task.spawn(function()
local req=(syn and syn.request)or(http and http.request)or http_request or request or(fluxus and fluxus.request)
if not req then Resp:SetDesc("Executor error") return end
local s,res=pcall(function()return req({Url="https://mirrorskey-system.vercel.app/api/chat",Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({prompt=PT,language=Lang})})end)
if not s or not res or res.StatusCode~=200 then Resp:SetDesc("Request failed.") return end
local ok,d=pcall(function()return HttpService:JSONDecode(res.Body)end)
Resp:SetDesc(ok and d.reply or "Invalid response.")
end)
end})
