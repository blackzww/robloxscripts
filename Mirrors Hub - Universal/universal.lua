...
if IsOPAuthorized then
    OP:Section({ Title = L("SectionPrivate"), Box = false, TextSize = 17 })
    ...
    OP:Button({
        ...
        Locked = false,
        Callback = function()
            Notify(L("OPConfirmed"), "flame", 3)
        end
    })

    OP:Button({
        Title = L("OPPlaceholder"),
        Desc = L("OPPlaceholderDesc"),
        Locked = false,
        Callback = function()
            Notify(L("NotConfigured"), "zap", 3)
        end
    })
end
