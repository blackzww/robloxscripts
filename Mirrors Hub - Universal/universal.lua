...
local function bootNotify(txt)
	...
	pcall(function()game:GetService("StarterGui"):SetCore("SendNotification",{Title="Mirrors Hub",Text=txt,Duration=4})end)
end
...
local function fakeLag(dt)
	...
	if lagT>=TrollC.FakeLagRate then
		...
		if r then
			...
			r.Anchored=true
			task.delay(TrollC.FakeLagHold,function()if r then r.Anchored=old end end)
		end
	end
end
