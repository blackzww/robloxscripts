	...
	for _=1,4 do
		...
		for _,v in ipairs(s.data)do
			if v.id and v.id~=game.JobId and v.playing and v.maxPlayers and v.playing<v.maxPlayers then
				...
				if small and (not best or v.playing<best.playing)then best=v end
			end
		end
		cursor=s.nextPageCursor
		if not cursor or #candidates>=30 then break end
	end
...
RunService.RenderStepped:Connect(function(dt)
	...
	antiVoidGuard();antiFling();god();fakeLag(dt);updateSpin(dt);updateOrbit(dt);updateAnnoy(dt);updateAim()
