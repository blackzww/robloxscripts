...
local TrollC={FakeLag=false,FakeLagRate=.22,FakeDeath=false,Orbit=false,OrbitRadius=7,OrbitSpeed=8,OrbitHeight=4,OrbitMode="Circle",Spin=false,SpinSpeed=35,HeadSit=false,Annoy=false,AnnoyMode="Circle",AnnoySpeed=.05,Ghost=true}
...
local function antiFling()
	...
	if OpC.AntiFling and r.AssemblyLinearVelocity.Magnitude>140 then r.AssemblyLinearVelocity=Vector3.new(r.AssemblyLinearVelocity.X,0,r.AssemblyLinearVelocity.Z) end
end
	...
	flingThread=task.spawn(function()
		...
		while flingRun do
			...
			if c and r and h and h.Health>0 then
				...
				local oldVel=r.AssemblyLinearVelocity
				r.AssemblyAngularVelocity=Vector3.new(0,OpC.FlingPower,0)
				RunService.RenderStepped:Wait()
				if r and r.Parent then
					r.AssemblyLinearVelocity=Vector3.new(oldVel.X,mv,oldVel.Z)
					r.AssemblyAngularVelocity=Vector3.zero
				end
				mv=-mv
			end
...
local deathState={Active=false,Platform=false,AutoRotate=true}
...
local function fakeLag()
	...
	local r=root();if not r then return end
	local old=r.Anchored;r.Anchored=true;task.delay(math.clamp(TrollC.FakeLagRate*.45,.03,.16),function()if r and r.Parent then r.Anchored=old end end)
end
...
local function updateAnnoy(dt)
	...
	rr.CFrame=CFrame.new(tr.Position+off,tr.Position);ghostTrail()
end
