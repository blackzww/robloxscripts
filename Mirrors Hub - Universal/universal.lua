...
local HUB_NAME,HUB_VERSION,HUB_FOLDER,CONFIG_FOLDER,LANG_FILE="Mirrors Hub","2.0 Supreme","MirrorsHub","MirrorsHub/config","MirrorsHub/language.json"
local booting=true
local function folder()
	if makefolder then
		if not isfolder(HUB_FOLDER) then pcall(function()makefolder(HUB_FOLDER)end)end
...
HomeTitle="Mirrors Hub",HomeDesc="Universal script hub.\nVersion: ",LoadScript="Load Script",LoadScriptDesc="Placeholder.",NoScript="No script configured yet.",BetaTitle="Beta Notice",BetaDesc="This hub is still in beta and may contain bugs. If you find any issue, report it on Discord.",DiscordTitle="Community",DiscordDesc="Found bugs or have suggestions? Tell us on Discord so the hub can improve.",StatusTitle="Status",StatusDesc="Current build: Supreme Optimized Beta.",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersão: ",LoadScript="Carregar Script",LoadScriptDesc="Placeholder.",NoScript="Nenhum script configurado ainda.",BetaTitle="Aviso Beta",BetaDesc="Este hub ainda está em beta e pode conter alguns bugs. Se encontrar algum problema, avise no Discord.",DiscordTitle="Comunidade",DiscordDesc="Achou bugs ou tem sugestões? Conte no Discord para o hub melhorar.",StatusTitle="Status",StatusDesc="Build atual: Supreme Optimized Beta.",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersión: ",LoadScript="Cargar Script",LoadScriptDesc="Placeholder.",NoScript="No hay script configurado todavía.",BetaTitle="Aviso Beta",BetaDesc="Este hub todavía está en beta y puede tener algunos bugs. Si encuentras un problema, avisa en Discord.",DiscordTitle="Comunidad",DiscordDesc="¿Encontraste bugs o tienes sugerencias? Avísalo en Discord para mejorar el hub.",StatusTitle="Estado",StatusDesc="Build actual: Supreme Optimized Beta.",
...
loadLang()
T.English.FreezeTools="Freeze Tools";T.English.FreezeAll="Freeze All Players";T.English.FreezeAllDesc="Freezes all other players locally.";T.English.UnfreezeAll="Unfreeze Players";T.English.UnfreezeAllDesc="Restores frozen players.";T.English.FreezeSelected="Freeze Selected Player";T.English.FreezeSelectedDesc="Freezes only the selected player.";T.English.CopyJob="Copy Job ID";T.English.CopyJobDesc="Copies the current server JobId.";T.English.JoinJob="Join Job ID";T.English.JoinJobDesc="Teleports to the typed JobId.";T.English.JobInput="Job ID Input";T.English.JobInputDesc="Paste the target JobId here.";T.English.Copied="Copied.";T.English.InvalidJob="Invalid Job ID.";T.English.ConfigPath="Folder: MirrorsHub/config\nFile: universal-config.json"
T.Portuguese.FreezeTools="Congelar";T.Portuguese.FreezeAll="Congelar Todos";T.Portuguese.FreezeAllDesc="Congela todos os outros jogadores localmente.";T.Portuguese.UnfreezeAll="Descongelar Jogadores";T.Portuguese.UnfreezeAllDesc="Restaura jogadores congelados.";T.Portuguese.FreezeSelected="Congelar Jogador Selecionado";T.Portuguese.FreezeSelectedDesc="Congela apenas o jogador selecionado.";T.Portuguese.CopyJob="Copiar Job ID";T.Portuguese.CopyJobDesc="Copia o JobId do servidor atual.";T.Portuguese.JoinJob="Entrar por Job ID";T.Portuguese.JoinJobDesc="Teleporta para o JobId digitado.";T.Portuguese.JobInput="Input do Job ID";T.Portuguese.JobInputDesc="Cole o JobId alvo aqui.";T.Portuguese.Copied="Copiado.";T.Portuguese.InvalidJob="Job ID inválido.";T.Portuguese.ConfigPath="Pasta: MirrorsHub/config\nArquivo: universal-config.json"
T.Spanish.FreezeTools="Congelar";T.Spanish.FreezeAll="Congelar Todos";T.Spanish.FreezeAllDesc="Congela localmente a todos los otros jugadores.";T.Spanish.UnfreezeAll="Descongelar Jugadores";T.Spanish.UnfreezeAllDesc="Restaura jugadores congelados.";T.Spanish.FreezeSelected="Congelar Jugador Seleccionado";T.Spanish.FreezeSelectedDesc="Congela solo al jugador seleccionado.";T.Spanish.CopyJob="Copiar Job ID";T.Spanish.CopyJobDesc="Copia el JobId del servidor actual.";T.Spanish.JoinJob="Entrar por Job ID";T.Spanish.JoinJobDesc="Teletransporta al JobId escrito.";T.Spanish.JobInput="Input del Job ID";T.Spanish.JobInputDesc="Pega aquí el JobId objetivo.";T.Spanish.Copied="Copiado.";T.Spanish.InvalidJob="Job ID inválido.";T.Spanish.ConfigPath="Carpeta: MirrorsHub/config\nArchivo: universal-config.json"
...
local TrollC={FakeLag=false,FakeLagRate=.22,FakeLagEffect=.45,FakeDeath=false,Orbit=false,OrbitRadius=7,OrbitSpeed=8,OrbitHeight=4,OrbitMode="Circle",Spin=false,SpinSpeed=35,HeadSit=false,Annoy=false,AnnoyMode="Circle",AnnoySpeed=.05,Ghost=true}
...
local Window=WindUI:CreateWindow({Title=L("WindowTitle"),Icon="door-open",Author="by blackzw.mp3",Folder=CONFIG_FOLDER,Size=UDim2.fromOffset(580,460),MinSize=Vector2.new(560,350),MaxSize=Vector2.new(850,560),ToggleKey=Enum.KeyCode.H,Transparent=true,Theme="Mirrors Purple",Resizable=false,SideBarWidth=200,BackgroundImageTransparency=.42,HideSearchBar=false,ScrollBarEnabled=true,NewElements=true,User={Enabled=true,Anonymous=false}})
...
local function applyNoclip()
	local c=char();local r=root();local h=hum();if not c or not r then return end
	if not MiscC.Noclip then lastSafeCF=r.CFrame;return end
	...
	if r.Position.Y>-150 then lastSafeCF=r.CFrame end
	if r.Position.Y<-250 and lastSafeCF then r.CFrame=lastSafeCF*CFrame.new(0,5,0);r.AssemblyLinearVelocity=Vector3.zero end
	if UserInputService:IsKeyDown(Enum.KeyCode.Space) and h and h.FloorMaterial==Enum.Material.Air then
		r.AssemblyLinearVelocity=Vector3.new(r.AssemblyLinearVelocity.X,math.max(r.AssemblyLinearVelocity.Y,42),r.AssemblyLinearVelocity.Z)
	end
end
...
local function applyHit(p)
	...
	HitC.TargetPart="HumanoidRootPart"
	local c=p.Character;local part=c and c:FindFirstChild("HumanoidRootPart");if not part or not part:IsA("BasePart")then return end
	savePart(part);part.Size=Vector3.new(HitC.Size,HitC.Size,HitC.Size);part.Transparency=HitC.Transparency;part.Color=HitC.Color;part.Material=Enum.Material.Neon;part.CanCollide=false;part.Massless=true
end
...
local function god()
	...
	if OpC.GodInf then
		h.BreakJointsOnDeath=false;h.RequiresNeck=false;h.MaxHealth=math.huge;h.Health=math.huge
		if c and not c:FindFirstChild("MirrorsInvincible")then local ff=Instance.new("ForceField");ff.Name="MirrorsInvincible";ff.Visible=false;ff.Parent=c end
	elseif OpC.God100 then
		h.BreakJointsOnDeath=false;h.MaxHealth=math.max(100,h.MaxHealth);if h.Health<100 then h.Health=100 end
	else
		local ff=c and c:FindFirstChild("MirrorsInvincible");if ff then ff:Destroy()end
	end
end
local function antiFling()
	...
	if r.AssemblyLinearVelocity.Magnitude>140 then r.AssemblyLinearVelocity=Vector3.new(r.AssemblyLinearVelocity.X,math.clamp(r.AssemblyLinearVelocity.Y,-20,35),r.AssemblyLinearVelocity.Z) end
end
	...
	flingThread=task.spawn(function()
		...
		while flingRun do
			...
			if r and h and h.Health>0 then
				...
				r.AssemblyAngularVelocity=Vector3.new(0,OpC.FlingPower*pulse,0)
				r.AssemblyLinearVelocity=Vector3.new(v.X,v.Y,v.Z)
				pulse=-pulse
			end
		end
		local r=root();if r then r.AssemblyAngularVelocity=Vector3.zero end
	end)
end

local ghostFolder,lagTick,orbitTick,annoyTick=nil,0,0,0
local deathState={Active=false,Platform=false,AutoRotate=true,Last=0}
local function playerNames()
	local t={"None"}
	for _,p in ipairs(Players:GetPlayers())do if p~=LP then t[#t+1]=p.Name end end
	table.sort(t,function(a,b)if a=="None"then return true elseif b=="None"then return false else return a:lower()<b:lower()end end)
	return t
end
...
local function tpToPlayer()
	local p=targetPlayer();local rr=root();local tr=p and root(p)
	if not p or not rr or not tr then notify(L("NoPlayer"),"user-x",3);return end
	rr.CFrame=tr.CFrame*CFrame.new(0,0,3);notify(L("Teleported"),"map-pin",2)
end
local function ensureGhostFolder()
	if ghostFolder and ghostFolder.Parent then return ghostFolder end
	ghostFolder=Instance.new("Folder");ghostFolder.Name="MirrorsHubGhosts";ghostFolder.Parent=workspace;return ghostFolder
end
local function ghostTrail()
	if not TrollC.Ghost then return end
	local r=root();if not r then return end
	local g=Instance.new("Part")
	g.Name="Ghost";g.Anchored=true;g.CanCollide=false;g.CanTouch=false;g.CanQuery=false;g.CastShadow=false;g.Material=Enum.Material.Neon;g.Transparency=.72;g.Size=Vector3.new(2,2.7,1);g.CFrame=r.CFrame;g.Color=Color3.fromRGB(134,0,212);g.Parent=ensureGhostFolder()
	task.delay(.45,function()if g then g:Destroy()end end)
end
local function fakeDeath()
	if tick()-deathState.Last<1.2 then return end;deathState.Last=tick()
	local h=hum();local r=root();if not h or not r then return end
	deathState.Active=true;deathState.Platform=h.PlatformStand;deathState.AutoRotate=h.AutoRotate
	h.PlatformStand=true;h.AutoRotate=false;h:ChangeState(Enum.HumanoidStateType.Physics)
	r.AssemblyLinearVelocity=Vector3.new(math.random(-35,35),65,math.random(-35,35));r.AssemblyAngularVelocity=Vector3.new(25,35,25)
end
local function recoverDeath()
	if tick()-deathState.Last<.35 then return end
	local h=hum();local r=root();deathState.Active=false
	if h then h.PlatformStand=deathState.Platform;h.AutoRotate=deathState.AutoRotate;h.Sit=false;h:ChangeState(Enum.HumanoidStateType.GettingUp)end
	if r then r.AssemblyAngularVelocity=Vector3.zero end
end
local function fakeLag()
	if not TrollC.FakeLag then return end
	local now=tick();if now-lagTick<TrollC.FakeLagRate then return end;lagTick=now
	local r=root();if not r then return end
	local old=r.Anchored;r.Anchored=true;task.delay(math.clamp(TrollC.FakeLagRate*TrollC.FakeLagEffect,.03,.28),function()if r and r.Parent then r.Anchored=old end end)
end
local function updateSpin(dt)
	if not TrollC.Spin then return end
	local r=root();if not r then return end
	r.CFrame=r.CFrame*CFrame.Angles(0,math.rad(TrollC.SpinSpeed)*(dt*60),0)
end
local function orbitOffset(a)
	local rad,hei=TrollC.OrbitRadius,TrollC.OrbitHeight
	if TrollC.OrbitMode=="UpDown"then return Vector3.new(math.cos(a)*rad,hei+math.sin(a*2)*rad,math.sin(a)*rad)end
	if TrollC.OrbitMode=="LeftRight"then return Vector3.new(math.sin(a)*rad,hei,0)end
	if TrollC.OrbitMode=="Diagonal"then return Vector3.new(math.cos(a)*rad,hei+math.cos(a)*rad,math.sin(a)*rad)end
	return Vector3.new(math.cos(a)*rad,hei,math.sin(a)*rad)
end
local function updateOrbit(dt)
	if not TrollC.Orbit then return end
	local p=targetPlayer();local rr=root();local tr=p and root(p);if not rr or not tr then return end
	orbitTick+=dt*TrollC.OrbitSpeed
	rr.CFrame=CFrame.new(tr.Position+orbitOffset(orbitTick),tr.Position)
end
local function updateAnnoy(dt)
	if not TrollC.Annoy then return end
	annoyTick+=dt;if annoyTick<TrollC.AnnoySpeed then return end;annoyTick=0
	local p=targetPlayer();local rr=root();local tr=p and root(p);if not rr or not tr then return end
	local a=tick()*12;local off
	if TrollC.AnnoyMode=="Cross"then local n=math.floor(tick()*10)%4;local pos={Vector3.new(4,1.5,0),Vector3.new(-4,1.5,0),Vector3.new(0,1.5,4),Vector3.new(0,1.5,-4)};off=pos[n+1]
	elseif TrollC.AnnoyMode=="Random"then off=Vector3.new(math.random(-5,5),math.random(1,5),math.random(-5,5))
	else off=Vector3.new(math.cos(a)*4,1.5,math.sin(a)*4)end
	rr.CFrame=CFrame.new(tr.Position+off)*CFrame.Angles(0,select(2,rr.CFrame:ToOrientation()),0);ghostTrail()
end

local frozenParts={}
local function freezeOne(plr)
	if not plr or plr==LP or not plr.Character then return end
	for _,v in ipairs(plr.Character:GetDescendants())do
		if v:IsA("BasePart")then
			if not frozenParts[v]then frozenParts[v]={Anchored=v.Anchored,Velocity=v.AssemblyLinearVelocity,Angular=v.AssemblyAngularVelocity}end
			v.Anchored=true;v.AssemblyLinearVelocity=Vector3.zero;v.AssemblyAngularVelocity=Vector3.zero
		end
	end
end
local function freezeAll()for _,p in ipairs(Players:GetPlayers())do freezeOne(p)end end
local function unfreezeAll()for part,old in pairs(frozenParts)do if part and part.Parent then part.Anchored=old.Anchored;part.AssemblyLinearVelocity=old.Velocity or Vector3.zero;part.AssemblyAngularVelocity=old.Angular or Vector3.zero end;frozenParts[part]=nil end end
local JoinJobId=""
local function copyJobId()if setclipboard then setclipboard(game.JobId);notify(L("Copied"),"copy",2)else notify(game.JobId,"copy",5)end end
local function joinJobId()if not JoinJobId or #JoinJobId<8 then notify(L("InvalidJob"),"triangle-alert",3);return end;TeleportService:TeleportToPlaceInstance(game.PlaceId,JoinJobId,LP)end
...
addSection(Tabs.Scripts,"Home")
para(Tabs.Scripts,L("HomeTitle"),L("HomeDesc")..HUB_VERSION,"Blue")
para(Tabs.Scripts,L("BetaTitle"),L("BetaDesc"),"Orange")
para(Tabs.Scripts,L("DiscordTitle"),L("DiscordDesc"),"Red")
para(Tabs.Scripts,L("StatusTitle"),L("StatusDesc"),"Green")
btn(Tabs.Scripts,"LoadScript","LoadScriptDesc",function()notify(L("NoScript"),"info",3)end)
...
drop(Tabs.Misc,"HitboxPart","HitboxPartDesc","HitboxTargetPart",{"HumanoidRootPart"},"HumanoidRootPart",function(v)resetHit();HitC.TargetPart="HumanoidRootPart";updateHit()end)
...
addSection(Tabs.OP,"FreezeTools")
btn(Tabs.OP,"FreezeSelected","FreezeSelectedDesc",function()freezeOne(targetPlayer())end)
btn(Tabs.OP,"FreezeAll","FreezeAllDesc",freezeAll)
btn(Tabs.OP,"UnfreezeAll","UnfreezeAllDesc",unfreezeAll)
addSection(Tabs.Troll,"Utility")
tog(Tabs.Troll,"FakeLag","FakeLagDesc","TrollFakeLag",false,function(v)TrollC.FakeLag=v end)
slid(Tabs.Troll,"FakeLag","FakeLagDesc","TrollFakeLagRate",5,80,22,1,function(v)TrollC.FakeLagRate=v/100 end)
slid(Tabs.Troll,"FakeLag","FakeLagDesc","TrollFakeLagEffect",10,100,45,1,function(v)TrollC.FakeLagEffect=v/100 end)
...
drop(Tabs.Troll,"OrbitMode","OrbitModeDesc","TrollOrbitMode",{"Circle","UpDown","LeftRight","Diagonal"},"Circle",function(v)TrollC.OrbitMode=v end)
...
btn(Tabs.Config,"CopyJob","CopyJobDesc",copyJobId)
if Tabs.Config.Input then Tabs.Config:Input({Title=L("JobInput"),Desc=L("JobInputDesc"),Flag="ConfigJobId",Placeholder="JobId",Callback=function(v)JoinJobId=tostring(v or "")end}) elseif Tabs.Config.Textbox then Tabs.Config:Textbox({Title=L("JobInput"),Desc=L("JobInputDesc"),Flag="ConfigJobId",Placeholder="JobId",Callback=function(v)JoinJobId=tostring(v or "")end}) end
btn(Tabs.Config,"JoinJob","JoinJobDesc",joinJobId)
...
btn(Tabs.Config,"DestroyUI","DestroyUIDesc",function()clearESP();resetHit();unfreezeAll();stopFling();if FOVCircle then FOVCircle.Visible=false;pcall(function()FOVCircle:Remove()end)end;pcall(function()Window:Destroy()end)end)
...
LP.CharacterAdded:Connect(function()task.wait(.5);applyMove();god();if HitC.Enabled then updateHit()end;if TrollC.FakeDeath then fakeDeath()end;local h=hum();if h then h.HealthChanged:Connect(function()if OpC.God100 or OpC.GodInf then god()end end)end end)
...
do local h=hum();if h then h.HealthChanged:Connect(function()if OpC.God100 or OpC.GodInf then god()end end)end end
...
task.spawn(function()while task.wait(.06)do if EspC.Enabled then updateESP()end end end)
task.spawn(function()while task.wait(.35)do if HitC.Enabled then updateHit()end end end)
task.defer(function()
	task.wait(.5)
