...
addSection(Tabs.Troll,"Soon")
Tabs.Troll:Button({Title="Troll Action 1",Desc=L("NotConfigured"),Locked=false,Callback=function()notify(L("NotConfigured"),"info",2)end})
Tabs.Troll:Button({Title="Troll Action 2",Desc=L("NotConfigured"),Locked=false,Callback=function()notify(L("NotConfigured"),"info",2)end})
...
btn(Tabs.Config,"ResetSession","ResetSessionDesc",function()Aim.Enabled=false;Aim.FOVVisible=false;EspC.Enabled=false;MiscC.Noclip=false;MiscC.InfiniteJump=false;HitC.Enabled=false;OpC.God100=false;OpC.GodInf=false;OpC.AntiFling=false;OpC.TouchFling=false;stopFling();currentTarget=nil;clearESP();resetHit();if FOVCircle then FOVCircle.Visible=false end;notify(L("ResetDone"),"rotate-ccw",3)end)
...
LP.CharacterAdded:Connect(function()task.wait(.5);applyMove();if HitC.Enabled then updateHit()end end)
...
RunService.RenderStepped:Connect(function()
	...
	applyNoclip();antiFling();god();updateAim()
end)
