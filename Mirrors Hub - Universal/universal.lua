	...
	flingThread=task.spawn(function()
		...
		while flingRun do
			...
			if c and r and h and h.Health>0 then
				...
				local moveVel=Vector3.new(oldVel.X,0,oldVel.Z)
				r.AssemblyLinearVelocity=moveVel+Vector3.new(0,OpC.FlingPower,0)
				r.AssemblyAngularVelocity=Vector3.new(OpC.FlingPower,OpC.FlingPower,OpC.FlingPower)
				RunService.RenderStepped:Wait()
				if r and r.Parent then
					r.AssemblyLinearVelocity=oldVel+Vector3.new(0,mv,0)
					r.AssemblyAngularVelocity=Vector3.zero
				end
				RunService.Stepped:Wait()
				if r and r.Parent then
					r.AssemblyLinearVelocity=oldVel
					r.AssemblyAngularVelocity=Vector3.zero
				end
				mv=-mv
			end
