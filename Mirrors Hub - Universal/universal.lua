...
local TXT={
English={
WindowTitle="Mirrors Hub - Universal",OpenButton="Open Mirrors Hub",Scripts="Scripts",Aimbot="Aimbot",ESP="ESP",Misc="Misc",OP="OP",Troll="Troll",Config="Config",
...
HomeTitle="Mirrors Hub",HomeDesc="Universal script hub.\nVersion: 9.8 Clean Real",BetaTitle="Beta Notice",BetaDesc="This hub is still in beta and may contain bugs. Report issues on Discord.",DiscordTitle="Community",DiscordDesc="Found bugs or have suggestions? Send them on Discord.",StatusTitle="Status",StatusDesc="Build: 9.8 Clean Real.",
...
Saved="Config saved.",LoadedConfig="Config loaded.",ResetDone="Session reset.",Loaded="Loaded 100%.",Copied="Copied.",ServerErr="Failed to fetch servers.",NoServer="No smaller server found.",NotConfigured="Not configured yet."
},
Portuguese={
WindowTitle="Mirrors Hub - Universal",OpenButton="Abrir Mirrors Hub",Scripts="Scripts",Aimbot="Aimbot",ESP="ESP",Misc="Misc",OP="OP",Troll="Troll",Config="Config",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersão: 9.8 Clean Real",BetaTitle="Aviso Beta",BetaDesc="Este hub ainda está em beta e pode conter bugs. Avise no Discord.",DiscordTitle="Comunidade",DiscordDesc="Achou bugs ou tem sugestões? Manda no Discord.",StatusTitle="Status",StatusDesc="Build: 9.8 Clean Real.",
...
Saved="Config salva.",LoadedConfig="Config carregada.",ResetDone="Sessão resetada.",Loaded="Carregado 100%.",Copied="Copiado.",ServerErr="Erro ao buscar servidores.",NoServer="Nenhum servidor menor encontrado.",NotConfigured="Ainda não configurado."
},
Spanish={
WindowTitle="Mirrors Hub - Universal",OpenButton="Abrir Mirrors Hub",Scripts="Scripts",Aimbot="Aimbot",ESP="ESP",Misc="Misc",OP="OP",Troll="Troll",Config="Config",
...
HomeTitle="Mirrors Hub",HomeDesc="Hub universal de scripts.\nVersión: 9.8 Clean Real",BetaTitle="Aviso Beta",BetaDesc="Este hub todavía está en beta y puede tener bugs. Avísalo en Discord.",DiscordTitle="Comunidad",DiscordDesc="¿Bugs o sugerencias? Mándalos en Discord.",StatusTitle="Estado",StatusDesc="Build: 9.8 Clean Real.",
...
Saved="Config guardada.",LoadedConfig="Config cargada.",ResetDone="Sesión reseteada.",Loaded="Cargado 100%.",Copied="Copiado.",ServerErr="Error al buscar servidores.",NoServer="No se encontró servidor menor.",NotConfigured="Todavía no configurado."
}
}
...
local MiscC={WalkSpeed=16,JumpPower=50,Noclip=false,NoclipMode="Safe",NoclipPower=34,InfiniteJump=false,AntiAFK=false,AntiVoid=true,Fullbright=false,NoFog=false}
...
local function applyNoclip(dt)
	...
	if r.Position.Y<-80 then r.CFrame=CFrame.new(r.Position.X,25,r.Position.Z);r.AssemblyLinearVelocity=Vector3.zero end
	...
	if move.Magnitude>0 then move=move.Unit*math.max(MiscC.NoclipPower,MiscC.WalkSpeed)end
	if MiscC.NoclipMode=="Safe" then
		local y=v.Y
		if UIS:IsKeyDown(Enum.KeyCode.Space)then y=MiscC.NoclipPower end
		if UIS:IsKeyDown(Enum.KeyCode.LeftControl)then y=-MiscC.NoclipPower end
		r.AssemblyLinearVelocity=Vector3.new(move.X,y,move.Z)
	elseif MiscC.NoclipMode=="Float" then
		noclipFloat+=dt*4
		r.AssemblyLinearVelocity=Vector3.new(move.X,math.sin(noclipFloat)*2,move.Z)
	elseif MiscC.NoclipMode=="Classic" and move.Magnitude>0 then
		r.CFrame=r.CFrame+move*dt
	end
end
...
local function fakeLag(dt)
	...
	lagT+=dt
	if lagT>=TrollC.FakeLagRate then
		lagT=0
		local r=root()
		if r then
			local old=r.Anchored
			r.Anchored=true
			task.delay(TrollC.FakeLagHold,function()if r and TrollC.FakeLag then r.Anchored=old end end)
		end
	end
end
...
local function hop()TP:Teleport(game.PlaceId,LP)end
local function smallServer()
	local ok,s=pcall(function()return Http:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100"))end)
	if not ok or not s or not s.data then notify(L("ServerErr"),"triangle-alert",3)return end
	local best
	for _,v in ipairs(s.data)do if v.id~=game.JobId and v.playing<v.maxPlayers and (not best or v.playing<best.playing)then best=v end end
	if best then TP:TeleportToPlaceInstance(game.PlaceId,best.id,LP)else notify(L("NoServer"),"x",3)end
end
...
local function importConfig()
	...
	if ok then applyConfig(d);applyMove();updateHit();refreshESP();notify(L("LoadedConfig"),"folder-open",2)end
end
local function saveProfile()
	if fs()then writefile(profilePath(),Http:JSONEncode(currentConfig()))end
end
local function loadProfile()
	if fs()and isfile(profilePath())then local ok,d=pcall(function()return Http:JSONDecode(readfile(profilePath()))end);if ok then applyConfig(d);applyMove();updateHit();refreshESP()end end
end
...
section(Tabs.Scripts,"Home");para(Tabs.Scripts,L("HomeTitle"),L("HomeDesc"),"Blue");para(Tabs.Scripts,L("BetaTitle"),L("BetaDesc"),"Orange");para(Tabs.Scripts,L("DiscordTitle"),L("DiscordDesc"),"Red");para(Tabs.Scripts,L("StatusTitle"),L("StatusDesc"),"Green")
...
section(Tabs.Config,"ConfigFile");para(Tabs.Config,L("Config"),L("ConfigPath"),"Blue");btn(Tabs.Config,"SaveConfig",function()if UConfig then UConfig:Save()end;notify(L("Saved"),"save",3)end);btn(Tabs.Config,"LoadConfig",function()if UConfig then UConfig:Load()end;loadProfile();safeStart();applyMove();updateHit();notify(L("LoadedConfig"),"folder-open",3)end);btn(Tabs.Config,"ExportConfig",exportConfig);btn(Tabs.Config,"ImportConfig",importConfig);input(Tabs.Config,"ProfileName","default",function(v)ProfileName=tostring(v or "default")end);tog(Tabs.Config,"AutoSave","ConfigAutoSave",false,function(v)AutoSave=v end);btn(Tabs.Config,"ResetSession",function()Aim.Enabled=false;Aim.FOVVisible=false;EspC.Enabled=false;MiscC.Noclip=false;MiscC.InfiniteJump=false;MiscC.Fullbright=false;MiscC.NoFog=false;HitC.Enabled=false;OpC.God100=false;OpC.GodInf=false;OpC.AntiFling=false;OpC.TouchFling=false;TrollC.FakeLag=false;TrollC.FakeDeath=false;TrollC.Orbit=false;TrollC.Spin=false;TrollC.HeadSit=false;TrollC.Annoy=false;stopFling();fakeDeath(false);unfreezeAll();clearESP();resetHit();if FOVCircle then FOVCircle.Visible=false end;notify(L("ResetDone"),"rotate-ccw",3)end)
...
LP.CharacterAdded:Connect(function()task.wait(.5);applyMove();god();if HitC.Enabled then updateHit()end;if TrollC.FakeDeath then fakeDeath(true)end end)
...
RunService.RenderStepped:Connect(function(dt)
	...
	applyMove();antiVoidGuard();updateWorldVisuals();applyNoclip(dt);antiFling();god();fakeLag(dt);updateSpin(dt);updateOrbit(dt);updateAnnoy(dt);updateAim()
	if TrollC.HeadSit then local p=getSelected();local r=root();local tr=p and root(p);local h=hum();if r and tr then r.CFrame=tr.CFrame*CFrame.new(0,3.1,0);if h then h.Sit=true end end end
